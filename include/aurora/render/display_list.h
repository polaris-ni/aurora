...
class DisplayList {
  public:
    ...
    [[nodiscard]] auto empty() const -> bool { return m_cmds.empty(); }

    /// @brief 已录制的命令条数（性能诊断 / 计数门槛用：反映录制规模，与回放开销正相关）。
    [[nodiscard]] auto cmd_count() const -> std::size_t { return m_cmds.size(); }

    auto push_cmd(const DrawCmd &cmd) -> void { m_cmds.push_back(cmd); }

    /// @brief 变长数据入池，返回索引（供 DrawCmd 引用）。
    auto add_string(const std::string &s) -> int {
        m_str_pool.push_back(s);
        return static_cast<int>(m_str_pool.size()) - 1;
    }
    auto add_colors(const std::vector<Color> &v) -> int {
        m_color_pool.push_back(v);
        return static_cast<int>(m_color_pool.size()) - 1;
    }
    auto add_floats(const std::vector<float> &v) -> int {
        m_float_pool.push_back(v);
        return static_cast<int>(m_float_pool.size()) - 1;
    }
    auto add_font(const Font &f) -> int {
        m_font_pool.push_back(f);
        return static_cast<int>(m_font_pool.size()) - 1;
    }
    auto add_image(const Image &img) -> int {
        m_image_pool.push_back(img);
        return static_cast<int>(m_image_pool.size()) - 1;
    }
    auto add_matrix(const Matrix2D &m) -> int {
        m_matrix_pool.push_back(m);
        return static_cast<int>(m_matrix_pool.size()) - 1;
    }
  ...
  private:
    ...
    std::vector<std::string> m_str_pool;
    std::vector<std::vector<Color>> m_color_pool;
    std::vector<std::vector<float>> m_float_pool;
    std::vector<Font> m_font_pool;
    std::vector<Image> m_image_pool;
    std::vector<Matrix2D> m_matrix_pool;
};
