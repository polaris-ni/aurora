  ...
  public:
    ...
    [[nodiscard]] static auto describe_static() -> WidgetDescriptor {
        return WidgetDescriptor{
            .name = "Grid",
            .properties = {
                { .name = "columns", .type = "int", .default_value = "1", .required = false, .note = "列数", .json_type = "integer", .enum_values = {}, .min_value = "1" },
                { .name = "gap", .type = "float", .default_value = "4.0", .required = false, .note = "单元格间距(px)", .json_type = "number", .enum_values = {}, .min_value = "0" },
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            sizes[i] = m_children[i].widget().layout(cc, ctx);
        }
        ...
        for (int col = 0; col < cols; ++col) {
            ...
            for (int row = 0; row < rows; ++row) {
                ...
                if (idx < n) {
                    m_children[idx].set_bounds(Rect{ .origin = Point{ .x = x, .y = y }, .size = sizes[idx] });
                }
                y += row_h[row] + g;
            }
            x += col_w[col] + g;
