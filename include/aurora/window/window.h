...
///   上传；需 `AURORA_BACKEND_GPU_WGPU` + 宿主后端：Win32 或 X11）。不可用时返回
...
class Window {
  public:
    ...
    auto set_title(std::string t) -> void {
        title_ = std::move(t);
        if (surface_) {
            surface_->set_title(title_);  // 下发到后端；Headless/GLFW 为空实现，Win32 经 SetWindowText 生效
        }
    }
    ...
    auto pump_events() const -> void { surface_->poll_platform_events(); }
    ...
    [[nodiscard]] auto has_pending_dirty() const -> bool {
        ...
        return first_frame_ || layout_dirty_ || !dirty_.is_empty();
    }
  ...
  public:
    ...
    AURORA_MAIN_THREAD auto run(const std::function<void()> &on_frame, int max_frames = -1) -> void {
        ...
        while (!should_close()) {
            ...
            const double wait_ms = next_wait_ms_;
            next_wait_ms_ = 0.0;  // 一次性消费：未重新设置则不等待（旧行为兜底）
            if (wait_ms != 0.0 && !should_close()) {
                const auto t0 = std::chrono::steady_clock::now();
                surface_->wait_events(wait_ms);
  ...
  private:
    ...
    std::weak_ptr<Widget> sink_root_;
    ...
    auto register_dirty_boundary(std::weak_ptr<Widget> b) -> void {
        ...
        for (const auto &e : dirty_boundaries_) {
            if (e.lock().get() == raw) {
                return;  // 去重（按被控对象指针）
            }
        }
        dirty_boundaries_.push_back(std::move(b));
    }
    ...
    auto render_hud(const BuildContext &ctx) -> void {
        ...
        if (!hud_painter_) {
            hud_painter_ = std::make_unique<Painter>();
        }
        if (hud_painter_->width() != w || hud_painter_->height() != h || hud_scale_ != s) {
            hud_painter_->set_scale(s);
            hud_painter_->begin(w, h);  // 逻辑 dp → 内部按 scale 放大为物理缓冲
            hud_scale_ = s;
        } else {
            hud_painter_->clear_rect(
                Rect{.origin = Point{.x = 0.0F, .y = 0.0F}, .size = sz});  // 清回透明零基底，避免与上帧叠加
        }
        // 叠加层控件在 on_paint 内以逻辑坐标绘制；bounding 用整窗逻辑尺寸（与 composite 1:1 对齐）。
        overlay_->paint(*hud_painter_, Rect{.origin = Point{.x = 0.0F, .y = 0.0F}, .size = sz}, ctx);
        hud_last_refresh_ = std::chrono::steady_clock::now();
        hud_rendered_ = true;
    }
    ...
    auto wire_present_request_once() -> void {
        ...
        surface_->set_present_request([this]() -> void {
            if (presenting_ || !cached_root_) {
                return;  // 防重入：渲染期间抓到的系统消息不得递归重渲染
            }
            // 系统要求的重绘（WM_PAINT/WM_SIZE）：窗口表面可能已失效（最小化还原、
            // 遮挡揭开），即便脏追踪判定整帧跳过，也必须重新上屏兜底（见下方跳帧分支）。
            system_redraw_ = true;
            (void)present_root(cached_root_);
            system_redraw_ = false;
        });
        present_wired_ = true;
    }
    ...
    [[nodiscard]] auto evaluate_dirty_plan(const Node &root, bool root_changed, FramePlan &plan)
        ...
        if (!plan.do_layout) {
            const float s = surface_->scale_factor();
            for (const Rect &r : dirty_.rects()) {
                plan.dirty_dev.push_back(Rect{.origin = Point{.x = r.origin.x * s, .y = r.origin.y * s},
            ...
            if (!dirty_.is_full() && !dirty_.is_empty()) {
        ...
        AURORA_PROFILE_SET(dirty_area_ratio, [this]() -> double {
            const Size vp = size();
            if (vp.width <= 0.0F || vp.height <= 0.0F || dirty_.is_empty()) {
                return 0.0;
            }
            if (dirty_.is_full()) {
                return 1.0;
            }
            const Rect mb = dirty_.merged_bounds();
    ...
    [[nodiscard]] auto prepare_context(Node &root, bool root_changed) -> BuildContext {
        ...
        if (sink_root_.lock().get() != root.operator->()) {
            install_dirty_sink(root.widget());
            sink_root_ = root.widget().weak_from_this();
        }
        return ctx;
    }
        ...
        const auto t_layout_start = std::chrono::steady_clock::now();
        {
            ...
            if (plan.do_layout) {
                if (plan.whole_tree_relayout) {
                    ...
                    for (const auto &wp : dirty_boundaries_) {
                        auto sp = wp.lock();
                        if (!sp) {
    ...
    [[nodiscard]] auto run_paint(Painter &p, Node &root, const BuildContext &ctx, const FramePlan &plan) const
        ...
        const auto t_paint_start = std::chrono::steady_clock::now();
        {
            ...
            if (plan.clip_logical.size.width > 0.0F && plan.clip_logical.size.height > 0.0F) {
                ...
                if (const Color cc = surface_->clear_color(); cc.a > 0) {
    ...
    [[nodiscard]] auto finish_present(const FramePlan &plan, bool hud_refreshed, double layout_ms,
        ...
        Result<bool> result{true};
        {
            ...
            surface_->set_present_dirty((overlay_ && hud_refreshed) ? std::vector<Rect>{} : plan.dirty_dev);
            result = present();
        }
