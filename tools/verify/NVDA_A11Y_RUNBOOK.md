...
> 时不认 `interface` 关键字（`win32_ua.cpp` / `win32_window.cpp` 编不过）。**构建验证探针必须用 mingw64 g++**：
...
探针窗会显示一个含 5 个控件的列：Button「确定」、Checkbox、Slider、TextInput「abc」、Text「订单总额」。
**不要让探针窗失去焦点中途退出**——它存续期间桥才在投影。
