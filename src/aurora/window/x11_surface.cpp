...
auto mods_from_state(unsigned int state) -> ModifierKey {
    ...
    if ((state & ShiftMask) != 0U) m = m | ModifierKey::Shift;
    if ((state & ControlMask) != 0U) m = m | ModifierKey::Control;
    if ((state & Mod1Mask) != 0U) m = m | ModifierKey::Alt;
    if ((state & Mod4Mask) != 0U) m = m | ModifierKey::Meta;
    return m;
}
...
X11Surface::X11Surface(int w, int h, const std::string &title, const WindowStyleOptions &style)
    ...
    if (d.dpy == nullptr) {
        AURORA_LOG_WARN("window", "X11Surface: XOpenDisplay failed (DISPLAY unset or unreachable); "
...
X11Surface::~X11Surface() {
    ...
    if (d.dpy != nullptr) {
        if (d.ic != nullptr) XDestroyIC(d.ic);
        if (d.im != nullptr) XCloseIM(d.im);
        if (d.gc != nullptr) XFreeGC(d.dpy, d.gc);
        if (d.win != 0) XDestroyWindow(d.dpy, d.win);
        XCloseDisplay(d.dpy);
    }
...
auto X11Surface::is_available() const -> bool { return m_impl->dpy != nullptr && m_impl->win != 0; }
...
auto X11Surface::begin_frame(int width, int height) -> Result<bool> {
    ...
    if (d.dpy != nullptr && d.win != 0) {
        ...
        if (XGetGeometry(d.dpy, d.win, &root_ret, &xr, &yr, &cw, &ch, &bw, &depth) != 0) {
            if (cw > 0) phys_w = static_cast<int>(cw);
            if (ch > 0) phys_h = static_cast<int>(ch);
        }
    }
    if (phys_w <= 0) phys_w = 1;
    if (phys_h <= 0) phys_h = 1;
...
auto X11Surface::painter() -> Painter & { return m_impl->painter; }
...
auto X11Surface::capture_window(const std::string &path) -> Result<bool> {
    ...
    if (!XGetWindowAttributes(d.dpy, d.win, &wa)) {
        return Result<bool>{ make_error(ErrorCode::GeneralNotSupported,
                                        "capture_window: XGetWindowAttributes failed") };
    }
...
auto X11Surface::size() const -> Size { return m_impl->size; }

auto X11Surface::scale_factor() const -> float { return m_impl->scale; }

auto X11Surface::should_close() const -> bool { return m_impl->close_requested; }

auto X11Surface::set_present_dirty(const std::vector<Rect> &device_rects) -> void {
    m_impl->present_dirty = device_rects;
}

auto X11Surface::set_event_handler(const EventHandler &h) -> void { m_impl->handler = h; }
...
auto X11Surface::native_handle() const -> void * {
    return reinterpret_cast<void *>(static_cast<std::uintptr_t>(m_impl->win));
}
...
auto X11Surface::poll_platform_events() -> void {
    ...
    while (XPending(d.dpy) > 0) {
        ...
        case ButtonRelease: {
            ...
            if (btn >= 4 && btn <= 7) {
                ...
                if (press && d.handler) {
                    ScrollEvent se;
                    se.position =
                        Point{ static_cast<float>(ev.xbutton.x) / d.scale, static_cast<float>(ev.xbutton.y) / d.scale };
                    if (btn == 4) {
        ...
        case Expose:
            ...
            if (ev.xexpose.count == 0 && m_present_request) {
                m_present_request();
            }
            break;
        case FocusIn:
        ...
        case ClientMessage:
            ...
            break;
        case DestroyNotify: d.close_requested = true; break;
        default: break;
        }
    }
}
