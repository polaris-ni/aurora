...
enum class Attr : std::uint8_t {
    ...
    InputError = 4,  ///< 输入错误段（下划线呈波浪）
    FixedConverted = 5,  ///< 已锁定、不再参与转换
};
...
///
/// 目标段 = 连续的 `TargetConverted` / `TargetNotConverted` 单元。IMM32 实际只给单段目标；
...
[[nodiscard]] auto make_preedit_state(std::u16string_view comp, std::size_t caret,
...
/// @return 填好 `preedit` / `cursor_index` / `sel_start` / `sel_end` 的事件（`committed` 留空）。
[[nodiscard]] auto make_preedit_state_utf8(std::string_view preedit, std::int64_t cursor_byte,
                                           std::int64_t sel_begin_byte, std::int64_t sel_end_byte)
