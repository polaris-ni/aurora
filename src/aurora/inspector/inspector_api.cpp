...
auto Inspector::simulate_drag(Widget &w, float dx, float dy) -> Result<void> {
    SimFocusContext focus;
    FocusManager *fm = focus.resolve(w);
    const Point center = center_of(w);

    // 与 simulate_click 同口径的前置命中测试：起点不可命中则不进入派发
    // （否则 Move/Release 会在无目标控件上空转，疑似「拖了但没反应」）。
    const Rect root_rect{.origin = Point{}, .size = w.size()};
    if (w.hit_test_chain(center, root_rect, BuildContext{}).empty()) {
        return make_error(ErrorCode::GeneralNotSupported,
                          "simulate_drag: target widget has no hit-testable area at its center");
    }

    // 一次完整拖拽 = Press（中心）→ Move（终点）→ Release（终点）。
    MouseEvent press;
    press.action = MouseAction::Press;
    press.button = MouseButton::Left;
    press.position = center;
    ...
    move.position = Point{.x = center.x + dx, .y = center.y + dy};
    EventDispatcher::dispatch(w, move, fm);
