...
auto is_bidi_format_control(char32_t cp) -> bool {
    // 与 `bidi_class_of` 的分类严格一致（见该函数内 LRE..RLO / LRI..PDI / LRM·RLM 分支）：
    // - U+202A–U+202E：显式方向格式化（LRE RLE PDF LRO RLO）
    // - U+2066–U+2069：方向隔离符（LRI RLI FSI PDI）
