...
ImageCodecRegistry::ImageCodecRegistry() : impl_(std::make_shared<Impl>()) {
    // 基础编解码器（始终可用）
...
auto ImageCodecRegistry::register_codec(std::shared_ptr<ImageCodec> codec) const -> void {
    if (codec) {
        impl_->codecs.push_back(std::move(codec));
    }
}
