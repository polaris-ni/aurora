...
| `aurora_verify_atspi` | Linux AT-SPI2 桥与真实 a11y 总线的接缝：`Socket.Embed` 握手、树可见性、状态集 / 动作 / 几何 / 文本读回、`DoAction` 回灌宿主（`on_click` 真实执行） | **libatspi 客户端**（探针内起 `python3-gi` 子进程，与 Orca 同路径）从桌面树按 app/frame/button/entry/static 逐检查项断言并打印 `RES|pass/fail`；实测（2026-09-20，WSLg）Wayland 与 X11 两路 13/13 ALL PASS。运行需 `GI_TYPELIB_PATH`/`PYTHONPATH` 指向解包好的 gir 仓库（见源文件头注释） |
