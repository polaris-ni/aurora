...
| TC-DEBUG-004 | 2026-09-24 | Qoder Agent | PASS | | 执行前当前工作目录下 `aurora_debug/` 不存在；按 `Ctrl+S` 后 stdout 恰新增 `capture(framebuffer): OK -> aurora_debug/google_play_framebuffer.png`；文件出现且 3345083 字节，头 8 字节为 `89 50 4e 47 0d 0a 1a 0a`，`IHDR` 解出 1100×760（即客户区逻辑尺寸），尾部含 `IEND`；纯文件名确被解析进 `set_output_directory("aurora_debug")` 设定的目录，父目录自动创建（仓库根与 `build/` 两种 CWD 下各复现一次） | | 丢键根因已实测定位为「派发时刻用 `GetAsyncKeyState` 采样物理修饰键」与消息泵排队延迟的竞态（`src/aurora/window/win32_host.cpp` 的 `current_modifiers`）。时长扫描（同一实例，每事件后 1.2 s 间隔，命中数按 stdout 新增 `capture(framebuffer)` 计数）：4 个按键事件塞进**同一次** `SendInput` 批量注入（computer-use `press_key Control_L+s` 即此路径，`inputEventCount` 为 4）命中 **0/10**；拆成 4 次独立 `SendInput`、整串 chord 约 5 ms 命中 **7/10**，约 45 ms（Ctrl 按住 25 ms 后按 S）命中 **9/10**，`keybd_event` 约 5 ms 命中 **4/5**。决定性对照：把目标进程 UI 线程用 `NtSuspendProcess` 冻结后再注入 45 ms 的人手节奏 chord、随后解冻，命中 **0/10**，即消息全部堆在队列里、派发时物理 Ctrl 已抬起，热键静默丢弃。结论：这不只是合注入的坑——真人快速轻点（整串 chord 短于一帧约 21~30 ms）或界面卡顿时同样会丢，属产品侧竞态（宜改用队列相对态 `GetKeyState` 或 `WM_KEYDOWN` 的 lParam bit 30，或由宿主自行跟踪修饰键按下流）。执行时另须先取得窗口前台权，否则按键落到别的窗口。 |
...
| TC-DEBUG-006 | 2026-09-24 | Qoder Agent | PASS | | `Ctrl+P` 后先出现 `=== runtime info ===`，随后节标题恰 6 个且顺序为 `surface_state:` / `widget_tree:` / `perf_snapshot:` / `frame_phase_timeline:` / `why_trace:` / `diagnostics:`，各节均为二空格缩进 JSON；`surface_state` 含 `available` true、`width` 1100、`height` 760、`has_native_window` true、`scale_factor` 1.0、`clear_color` `[245,245,247,255]`、`should_close` false、`frame_count` 0；`widget_tree` 为递归树，根 `NavigatorHost` → 1 个 `Provider`，每层含 `type`/`props`/`children`；`perf_snapshot` 十一个键齐备；`frame_phase_timeline` 六个键齐备（含 `flamegraph`）；`why_trace` 条目含 `frame`/`kind`/`propagated`/`type`；`diagnostics` 逐字为 `{"count":0,"diagnostics":[]}` | | 两处按运行时补记：`why_trace` 另含 `total_recorded` 键，且 `count`(256) ≠ `len(entries)`（环形缓冲按上限截断）；六节 JSON 会被周期性 `FPS` 摘要行插断（同写 stdout），脚本解析须先过滤 `^FPS ` 行。`frame_count` 取值本用例未作断言，实跑恒为 0 而窗口正以约 33 FPS 出帧；现已查明属实现与契约不符：`include/aurora/window/win32_surface.h:135` 把它转发给宿主 `present_count()`，而该计数在 `src/aurora/window/win32_host.cpp:175` 自陈为「已触发同步重渲染次数」，自增点只有 WM_SIZE（同文件 509 行）与 WM_PAINT（526 行）两个分支，帧循环走的 `Win32Surface::present()` 完全不计数。复核数据：启动后读 0，静置 20 秒（其间新增 23 行 FPS 摘要，约合 660 帧真实出帧）后再读仍为 0；做一次放大 40×30 再还原的窗口 resize 后读到 4。其余后端（Headless 的 `surface.h` 447 行、Glfw、D3D11、WASM 与三个 wgpu 型）都在各自 `present()` 内 `++frame_`，故同名 JSON 键跨后端语义不一致。本用例不改预期，缺陷同日修复收口：`Win32Surface` 改持自有 `presented_frames_`（仅 `present()` 真正上屏时自增），`itest_win32_present` 锁「逐帧 +1 且宿主 `present_count()` 不动」；重建载体后实跑读数为开窗后 124、静置 10 秒（约 89 帧真实上屏）后 213，不再恒 0。口径差已收口：X11 / Wayland 两后端同日补计（X11 在 `XPutImage` 上屏后自增，Wayland 在 `attach` 加 `commit` 后自增，Hidden 档不 commit 故不计），`itest_x11_present` / `itest_wayland_present` 在 WSLg 真实窗口下逐帧断言 +1 通过，抽掉自增行两用例同红（证非空转）；仅 macOS 后端仍走基类默认恒报 0，本机无 Apple 设备故未验。 |
