...
> 架构/设计理念层条目见 `architecture/ARCHITECTURE_AI.md` §12。状态符号：✅ 已采纳 / 🟡 延后 / ❌ 不做（移出范围）。
...
### 一.10 二层属性划分（固有属性 + 正交 Modifier）✅ 已采纳
...
### 一.12 控件可定制性契约（主题回退 + 状态反馈 + protected 绘制钩子）✅ 已采纳（v0.24.0）
...
### 四.3 MCP / CLI ✅ 已采纳（v0.8.0）

stdio JSON-RPC 2.0 MCP Server（`aurora_mcp`，8 个 tools）+ CLI 工具（`aurora_cli`，8 个子命令）， 消费 describe ()
/list_all_schemas ()/validate ()/render_to_png ()/to_code () 等库 API。 LSP 已于 v0.11.0 落地。详见 `specification/subsystems_api/SUBSYSTEM_API_TOOLING.md`
（§H.17 LSP / §H.19 MCP）。
...
### 四.8 Playground / REPL ❌ 不做（移出范围）

不提供在线 Playground/REPL（用户明确移出范围）。

### 四.9 组件发现 ✅ 已采纳（v0.7.0）

每个控件实现 `static describe_static()`（注册表 / 工具链消费的完整 `WidgetDescriptor`，含属性 / 事件 / 子节点策略 /
