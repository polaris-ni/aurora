...
- **wgpu GPU 栅格后端（`WgpuRhi` + `WgpuSurface`）**：跨平台 GPU 主力路径——同一套 WGSL 管线经 wgpu-native（Rust 静态库，源码 vendored 于 `third_party/wgpu-native/`，cargo 构建）覆盖 Vulkan / D3D12 / Metal / GLES。命令消费面与 `GpuGlRhi` 实现同一 `RhiBackend` + `RhiFrameSink` 契约、语义同源（SDF 裁剪 / LUT 渐变 / PMA 图像 / 字形图集复用 / 层缓存 + 流式槽）；另有离屏模式（`read_pixels` 全帧读回，测试与探针通道）。宿主接线：`WgpuSurface` 复用 `Win32Window` 宿主做 GPU 直渲上屏（`end_frame` 内 present），失败永久回退 GDI 上传路径；`RendererPreference::GpuWgpu` 强制路由不降级。规格见 `specification/03-layout-render.md` §8.8。
...
| `WgpuRhi`（`Surface::gpu_backend()`） | DisplayList 的 wgpu（WGSL）批渲染：与 `GpuGlRhi` 同一命令契约与批切分口径；宿主模式（Win32 HWND → swapchain）与离屏模式（内部纹理 + `read_pixels` 读回）双形态；后端 Auto 择链（Windows：D3D12 → Vulkan → GLES） | `AURORA_BACKEND_GPU_WGPU`（默认 OFF） |
...
`Win32Surface`、`D3D11Surface` 与 `WgpuSurface` 共用 `Win32Window` 宿主（创建 / 消息泵 / 事件翻译 / DPI / 同步重渲染 / 运行期标题），仅 present 后端不同。宿主采用 pimpl 隔离，故 `hwnd()` / `background_brush()` 以 `void*` 返回（调用方 `static_cast`）。窗口过程按**消息族**分派到各 `handle_*` 函数，取代单体 switch。
