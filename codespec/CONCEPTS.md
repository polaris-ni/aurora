...
| 12 | **Event（事件）** | `MouseEvent` `KeyEvent` `ScrollEvent` `TextInputEvent` `TextCompositionEvent`（IME 预编辑：preedit / 组合光标 / 待转换选区 / commit，经 `EventDispatcher` 只路由焦点控件的 `on_text_composition`） `TouchEvent` `FileDropEvent` `EventDispatcher` `TouchDispatcher` `FocusManager`（焦点作用域 `push_scope`/`pop_scope`：模态弹层 Tab 陷阱与焦点恢复） | `event/event.h` `event/dispatcher.h` `event/focus.h` |
