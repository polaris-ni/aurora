...
| 15 | **Accessibility（无障碍）** | `AccessibilityNode`（id / role / name / value / hint / bounds / actions / state / range / level / is_control / is_content） `AccessibilityRole` `AccessibilityAction` `AccessibilityState` `AccessibilityRange` `AccessibilityTextSelection` `infer_accessibility_role` `build_accessibility_tree`（含几何回填；语义由控件虚钩子 `Widget::accessibility_role` / `accessibility_label` / `accessibility_value` / `accessibility_hint` / `accessibility_state` / `accessibility_range` / `accessibility_level` / `accessibility_text` / `accessibility_selection` / `accessibility_scroll` 自填，可覆写） `Widget::explicit_accessibility_label` / `set_accessibility_label`（宿主侧显式读屏名，对标 `aria-label`，Name 回退链最高优先级） `Widget::runtime_id()`（稳定身份） `Widget::perform_accessibility_action`（读屏反向操作，默认路由真实事件路径） `Widget::announce` / `notify_accessibility_announcement`（Live Region 动态播报） `AccessibilitySettings`（`reduce_motion` / `high_contrast` / `font_scale` / `screen_reader_active`，经 Environment 注入或进程级默认） `AccessibilityEvent` / `AccessibilityEventKind` / `set_accessibility_event_handler`；平台桥层 `a11y::Provider` + `ProviderRegistry` + `TreeSnapshot` / `diff_snapshots` + `UtfOffsetMap`（Win32 UIA 实现在 `window/detail/win32_ua.*`） | `core/accessibility.h` `core/a11y_types.h` `core/a11y_provider.h` `core/a11y_text.h` `widget/widget.h` `widget/a11y_tree.h` `widget/a11y_diff.h` |
