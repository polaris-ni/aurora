...
| 15 | **Accessibility（无障碍）** | `AccessibilityNode`（role / name / value / hint / bounds / actions） `AccessibilityRole` `AccessibilityAction` `infer_accessibility_role` `build_accessibility_tree`（含几何回填；语义由控件钩子 `Widget::accessibility_label` / `accessibility_value` / `accessibility_hint` 自填，可覆写） `AccessibilitySettings`（`reduce_motion` / `high_contrast` / `font_scale` / `screen_reader_active`，经 Environment 注入或进程级默认） `AccessibilityEvent` / `AccessibilityEventKind` / `set_accessibility_event_handler` | `core/accessibility.h` `widget/widget.h` |
