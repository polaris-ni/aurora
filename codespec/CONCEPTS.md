...
| 17 | **Render（渲染）** | `Painter` `Surface` `HeadlessSurface` `Win32Surface` `D3D11Surface` `WgpuSurface` `GlfwSurface` `X11Surface` `WaylandSurface` `MacOSSurface` `WasmSurface` `create_window`（工厂） `auto_detect_surface` | `render/painter.h` `window/*.h` |
