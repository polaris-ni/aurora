...
| 12 | **Event（事件）** | `MouseEvent` `KeyEvent` `ScrollEvent` `TextInputEvent` `TextCompositionEvent`（IME 预编辑：preedit / 组合光标 / 待转换选区 / commit，经 `EventDispatcher` 只路由焦点控件的 `on_text_composition`） `TouchEvent` `FileDropEvent` `EventDispatcher` `TouchDispatcher` `FocusManager` | `event/event.h` `event/dispatcher.h` `event/focus.h` |
