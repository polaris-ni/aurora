...
AURORA_TEST() {
    ...
    // 场景 1：replay 保真 + 跳过子树遍历。
    {
        ...
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 1, "frame1 records subtree (on_paint once)");
        ...
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 1,
    ...
    // 场景 2：内容变脏触发祖先 DL 失效并重录。
    {
        ...
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 1, "before-change records once");

        c->mark_needs_paint();  // 内容脏：须沿布局父链失效祖先 DL 并重录
        AURORA_TEST_CHECK(win.present_root(root).ok());
        auto after = copy_pixels(hs.data(), n);
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 2, "dirty triggers re-record (on_paint called again)");
    ...
    // 场景 3：离屏合成（cache_layer）Composite 命令录制/回放。
    {
        ...
        auto &hs = dynamic_cast<HeadlessSurface &>(win.surface());
        auto f1 = copy_pixels(hs.data(), n);
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 1, "cache_layer frame1 records once");

        win.force_full_redraw();
        AURORA_TEST_CHECK(win.present_root(root).ok());  // 帧2：仅回放（命中 DL，Composite 命令重贴离屏缓冲）
        auto f2 = copy_pixels(hs.data(), n);
        AURORA_TEST_CHECK_MSG(c->paint_calls_ == 1, "cache_layer frame2 DL-hit skips subtree");
