// aurora_test_harness.h — Aurora 轻量级注册式测试框架（v2，单头、零三方依赖）。
//
// 与旧版（全局失败计数 + 各文件自带 main）的差异：
//   - 用例通过 AURORA_TEST() 注册进进程级注册表，main 由框架唯一提供（aurora_test_main.cpp），
...
//
// 构建/运行（详见 cmake/AuroraTests.cmake）：
...
// 测试 TU 统一提供推荐短别名 `au`，避免各文件重复声明；
// 与 aurora.h 中的全局别名同名同目标，重复定义合法（namespace alias 可重声明至同一命名空间）。
namespace au = aurora;
...
using au::testing::near_d;
using au::testing::near_f;
