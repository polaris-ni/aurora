...
AURORA_TEST() {
    ...
    AURORA_TEST_CHECK_MSG(p0->paint_calls_ > 0, "p0 in viewport is painted");
    AURORA_TEST_CHECK_MSG(p1->paint_calls_ > 0, "p1 in viewport is painted (touches bottom edge)");
    AURORA_TEST_CHECK_MSG(p2->paint_calls_ == 0, "p2 outside viewport is culled (skipped)");
    AURORA_TEST_CHECK_MSG(p3->paint_calls_ == 0, "p3 outside viewport is culled (skipped)");
    ...
    Node root_scroll{Scroll{ScrollProps{.child = Node{Column{Node{s0}, Node{s1}, Node{s2}, Node{s3}}}}}};
    Window win_scroll = make_window(100, 100);
    AURORA_TEST_CHECK(win_scroll.present_root(root_scroll).ok());
    AURORA_TEST_CHECK_MSG(s0->paint_calls_ > 0, "Scroll: in-view child is painted");
    AURORA_TEST_CHECK_MSG(s1->paint_calls_ > 0, "Scroll: child touching bottom edge is painted");
    AURORA_TEST_CHECK_MSG(s2->paint_calls_ > 0,
                          "Scroll: off-viewport child is also painted (full-page offscreen buffer)");
    AURORA_TEST_CHECK_MSG(s3->paint_calls_ > 0,
