...
struct StatsSnapshot {
    double avg_frame_ms_;
    double avg_layout_ms_;
    double avg_paint_ms_;
    double fps_;
    double worst_frame_ms_;
    double p99_;
...
auto run_benchmark(int widget_count, const int frames) -> StatsSnapshot {
    ...
    return StatsSnapshot{
        .avg_frame_ms_ = fs.avg_frame_ms(),
        .avg_layout_ms_ = fs.avg_layout_ms(),
        .avg_paint_ms_ = fs.avg_paint_ms(),
        .fps_ = fs.fps(),
        .worst_frame_ms_ = fs.worst_frame_ms(),
        .p99_ = fs.percentile_ms(0.99),
        .total_frames_ = fs.total_frames(),
    };
}
...
AURORA_TEST() {
    ...
    for (int i = 0; i < 4; ++i) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
        ...
        AURORA_TEST_PRINTF("N=%-3d  avg=%.1fms layout=%.1fms paint=%.1fms fps=%.0f worst=%.1fms p99=%.1fms\n", n,
                           s.avg_frame_ms_, s.avg_layout_ms_, s.avg_paint_ms_, s.fps_, s.worst_frame_ms_, s.p99_);

        // 每个规模都能正常完成帧循环
        AURORA_TEST_CHECK_MSG(s.total_frames_ > 0, "total_frames > 0");
    }

    // 500 按钮场景 avg_frame_ms < 50ms（宽松阈值，确保不超时）
    AURORA_TEST_CHECK_MSG(snapshots[3].avg_frame_ms_ < 50.0, "N=500 avg_frame_ms < 50ms");

    // 增长趋势检查：更大规模不应比小规模慢一个数量级以上（宽松）
    for (int i = 1; i < 4; ++i) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
        const double ratio = snapshots[i].avg_frame_ms_ / (snapshots[i - 1].avg_frame_ms_ + 0.001);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
