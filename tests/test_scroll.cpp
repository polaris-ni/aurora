...
static void run() {
    ...
    constexpr Constraints c{.min = Size{.width = 0.0F, .height = 0.0F}, .max = Size{.width = 200.0F, .height = 100.0F}};
    ...
    ScrollEvent se;
    se.delta_y_ = -24.0F;
    se.is_handled_ = false;
    multi.on_scroll(se);
    AURORA_TEST_CHECK(se.is_handled_);
    ...
    const auto col = std::make_shared<Column>(std::initializer_list{Node{Text{"a"}}, Node{Text{"b"}}, Node{Text{"c"}}});
    ...
    ScrollEvent zero;
    zero.delta_y_ = 0.0F;
    zero.is_handled_ = false;
    tall.on_scroll(zero);
    AURORA_TEST_CHECK(zero.is_handled_);
...
struct ResizableStack : Widget {
    int m_rows_ = 1000;  ///< 逻辑高度（px），可随时改以触发内容尺寸变化
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext & /*ctx*/) -> Size override {
        ...
        return Size{.width = w, .height = static_cast<float>(m_rows_)};
    }
    auto on_paint(Painter &p, const Rect &bounds, const BuildContext & /*ctx*/) -> void override {
        ...
        for (int y = 0; y < m_rows_; ++y) {
            ...
            p.fill_rect(
                Rect{.origin = Point{.x = 0.0F, .y = static_cast<float>(by)}, .size = Size{.width = w, .height = 1.0F}},
                Color{g, g, g, 255});
        }
    }
};
...
auto paint_scroll(Node &root, BuildContext const &ctx, const int off = 0) -> Painter {
    ...
    Painter p;
    p.begin(100, 100);
    root->paint(p, Rect{.origin = Point{.x = 0.0F, .y = 0.0F}, .size = Size{.width = 100.0F, .height = 100.0F}}, ctx);
    (void)off;
    return p;
}
...
static void run() {
    ...
    child->m_rows_ = 1200;  // 内容尺寸变化：下帧 m_content_valid=false，但 m_buffer_origin_y 未更新
    {
        Painter p = paint_scroll(root, ctx);
...
[[nodiscard]] auto run_pair(int base_rows) -> Pair {
    Pair p;
    p.base_ = ScrollBenchHarness::run(build_scroll_column(base_rows),
                                      Size{.width = AURORA_VIEWPORT_W, .height = AURORA_VIEWPORT_H}, gate_cfg());
    p.x10_ = ScrollBenchHarness::run(build_scroll_column(base_rows * 10),
                                     Size{.width = AURORA_VIEWPORT_W, .height = AURORA_VIEWPORT_H}, gate_cfg());
    return p;
}
...
static void run() {
    ...
    AURORA_TEST_CHECK_MSG(p.x10_.trustworthy(), "x10: harness readings trustworthy");
    ...
    if constexpr (profiling_enabled()) {
        ...
        AURORA_TEST_CHECK_MSG(p.base_.full_redraw_frames() == 0,
                              "G-5: full_redraw_frames == 0/120 (offscreen-buffer composite)");
        AURORA_TEST_CHECK_MSG(p.x10_.full_redraw_frames() == 0, "G-5[x10]: full_redraw_frames == 0");

        // ---- 滚动帧 layout_nodes ≤ 可见项×1.25，且内容×10 不增长 ----
        // 保持既有阈值语义，改写 lround 会翻转边界期望值。
        // NOLINTNEXTLINE(bugprone-incorrect-roundings)
        AURORA_TEST_CHECK_MSG(p.base_.counters_max().layout_nodes <= static_cast<std::uint32_t>(g6_bound + 0.5),
                              "G-6: layout_nodes_max <= visible items x1.25");
        AURORA_TEST_CHECK_MSG(p.x10_.counters_max().layout_nodes <= p.base_.counters_max().layout_nodes,
                              "G-6[x10]: layout_nodes does not grow with content x10");

        // ---- dl_records ×10 波动 ≤ 10%（须正比于滚动距离而非内容总量）----
        const auto dl_base = p.base_.counters_sum().dl_records;
        const auto dl_x10 = p.x10_.counters_sum().dl_records;
        AURORA_TEST_CHECK_MSG(
            dl_x10 <= dl_base + static_cast<std::uint32_t>((static_cast<double>(dl_base) * 0.10) + 1.0),
            "G-7: dl_records x10 fluctuation <= 10%");

        // ---- Scroll 缓冲字节 ≤ 视口×4×(1+2×overscan)，且 ×10 不增长（滑动窗口）----
        AURORA_TEST_CHECK_MSG(p.base_.counters_max().scroll_buffer_bytes <= buf_allowed,
                              "G-8: scroll_buffer <= viewport x4 x(1+2xoverscan)");
        AURORA_TEST_CHECK_MSG(p.x10_.counters_max().scroll_buffer_bytes <= p.base_.counters_max().scroll_buffer_bytes,
                              "G-8[x10]: scroll_buffer does not grow with content x10");

        AURORA_TEST_PRINTF("  G-5  full_redraw base/x10 = %zu / %zu\n", p.base_.full_redraw_frames(),
                           p.x10_.full_redraw_frames());
        AURORA_TEST_PRINTF("  G-6  layout_nodes_max base/x10 = %u / %u (bound=%.1f)\n",
                           p.base_.counters_max().layout_nodes, p.x10_.counters_max().layout_nodes, g6_bound);
        AURORA_TEST_PRINTF("  G-7  dl_records_sum base/x10 = %u / %u\n", dl_base, dl_x10);
        AURORA_TEST_PRINTF("  G-8  scroll_buffer base/x10 = %llu / %llu (allowed=%llu)\n",
                           static_cast<unsigned long long>(p.base_.counters_max().scroll_buffer_bytes),
                           static_cast<unsigned long long>(p.x10_.counters_max().scroll_buffer_bytes),
...
constexpr float AURORA_K_LEAF_H = 60.0F;
constexpr Color AURORA_K_SKELETON_COLOR{210, 210, 210, 255};
constexpr Color AURORA_K_CONTENT_COLOR{255, 64, 0, 255};  ///< 高饱和：与骨架灰/底色可判别
...
struct SelfDrivingLeaf : LeafWidget {
    int paints_ = 0;
    Color color_ = AURORA_K_SKELETON_COLOR;
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext & /*ctx*/) -> Size override {
        return c.constrain(Size{.width = c.max.width, .height = AURORA_K_LEAF_H});
    }
...
struct SwitchingBody : Container {
    int phase_ = 0;  ///< 0=骨架 1=真实内容
    int paint_ticks_ = 0;  ///< 骨架期已绘制帧数
    int layout_calls_ = 0;  ///< on_layout 真实执行次数
    std::shared_ptr<SelfDrivingLeaf> skeleton_;
    std::shared_ptr<SelfDrivingLeaf> content_;  ///< 切换后**动态新建**，旧接线快照覆盖不到
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        ...
        if (phase_ == 0) {
            leaf->color_ = AURORA_K_SKELETON_COLOR;
            skeleton_ = leaf;
        } else {
            leaf->color_ = AURORA_K_CONTENT_COLOR;
            content_ = leaf;
        }
        Node n{leaf};
        n.widget().mount(ctx);
        ...
        n.widget().layout(
            Constraints{.min = Size{.width = w, .height = 0.0F}, .max = Size{.width = w, .height = AURORA_K_LEAF_H}},
            ctx);
        n.set_bounds(Rect{.origin = Point{.x = 0.0F, .y = 0.0F}, .size = Size{.width = w, .height = AURORA_K_LEAF_H}});
        m_children.push_back(std::move(n));
        return c.constrain(Size{.width = w, .height = AURORA_K_LEAF_H});
    }

    auto on_paint(Painter &p, const Rect &b, const BuildContext &ctx) -> void override {
        // 帧/时钟驱动的「延期布局」：骨架期结束后切真实内容（不经外部显式标脏，与 demo 同构）。
        if (phase_ == 0 && ++paint_ticks_ >= 3) {
            phase_ = 1;
            mark_needs_layout();
        }
...
static void run() {
    ...
    AURORA_TEST_PRINTF("layout_calls=%d phase=%d\n", body->layout_calls_, body->phase_);
    AURORA_TEST_CHECK_MSG(body->phase_ == 1, "frame-driven skeleton->real-content switch should have happened");
    AURORA_TEST_CHECK_MSG(body->layout_calls_ >= 2,
                          "switch must trigger re-layout, on_layout should run more than once");
    AURORA_TEST_CHECK_MSG(static_cast<bool>(body->content_),
    ...
    const int content_paints = body->content_->paints_;
    AURORA_TEST_PRINTF("content leaf paints=%d (total frames %d)\n", content_paints, k_frames);
    AURORA_TEST_CHECK_MSG(
        content_paints > 5,
    ...
    const long content_px = count_color(win.surface(), AURORA_K_CONTENT_COLOR, 8);
...
// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables) 测试帧计数全局，跨测试函数共享
std::atomic g_tick{0};
...
struct SelfDrivingLeaf : LeafWidget {
  ...
  protected:
    ...
    auto on_paint(Painter &p, const Rect &b, const BuildContext & /*ctx*/) -> void override {
        ...
        p.fill_rect(b, AURORA_PALETTE.at(idx));  // 绘制「当前帧」颜色
...
struct RootShell : Container {
    std::shared_ptr<Scroll> m_scroll_;
    std::shared_ptr<SelfDrivingLeaf> m_leaf_;
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        ...
        if (!m_scroll_) {
            m_leaf_ = std::make_shared<SelfDrivingLeaf>();
            m_scroll_ = std::make_shared<Scroll>(ScrollProps{.child = Node{m_leaf_}});
            m_children.clear();
            m_children.emplace_back(m_scroll_);
            for (auto &n : m_children) {
                n.widget().mount(ctx);
                n.widget().set_layout_parent(this);
            }
        }
        const float scroll_h = std::max(0.0F, h - AURORA_TOP_H);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
        // 容器类型无法本地确证为顺序容器，operator[] 与 .at() 语义不同（map/json 的 [] 会插入键）
        m_children[0].widget().layout(
            Constraints{.min = Size{.width = w, .height = scroll_h}, .max = Size{.width = w, .height = scroll_h}}, ctx);
        // Scroll 视口位于屏幕 (0, AURORA_TOP_H) —— 内容坐标 (0,0) 的后代 ≠ 此屏幕坐标。
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
        // 容器类型无法本地确证为顺序容器，operator[] 与 .at() 语义不同（map/json 的 [] 会插入键）
        m_children[0].set_bounds(
            Rect{.origin = Point{.x = 0.0F, .y = AURORA_TOP_H}, .size = Size{.width = w, .height = scroll_h}});
...
static void run() {
    ...
    AURORA_TEST_PRINTF("leaf paints=%d (total frames %d)\n", shell->m_leaf_->paints_, k_frames);
    AURORA_TEST_CHECK_MSG(shell->m_leaf_->paints_ > k_frames / 2,
    ...
    const int last = (k_frames - 1) % static_cast<int>(AURORA_PALETTE.size());
    const Color expect = AURORA_PALETTE.at(last);
    ...
    if (last != 0) {
        const long first_px = count_color(win.surface(), AURORA_PALETTE.at(0), 8);
