...
// ============================================================================
// 尾巴：TextInput 对 bidi 格式控制符的渲染侧确定性支持
// ----------------------------------------------------------------------------
// UBA 解析（uba_levels / uba_visual_order）已把 LRE/RLE/PDF/LRO/RLO/LRI/RLI/FSI/PDI
// 与 LRM/RLM 当零宽不可见格式标记参与层级计算；但渲染侧此前依赖字体恰好提供零推进字形
// （捆绑 Noto 偶然正确），字体无关性无保证——一旦换用无对应字形的字体，控制符会落出
// .notdef 豆腐盒、占出非零宽度。本组用例锁定：控制符在度量与绘制侧**确定为零宽零墨迹**，
// 且不改变相邻可见文本的宽度/墨迹。
// ============================================================================

AURORA_TEST_CASE(bidi_format_control_classification) {
    // 纯函数判定覆盖全部九类显式方向符 + LRM/RLM；并确认普通标点/字母/阿文不在其列。
    using aurora::render::detail::is_bidi_format_control;
    AURORA_TEST_CHECK(is_bidi_format_control(0x202AU));  // LRE
    AURORA_TEST_CHECK(is_bidi_format_control(0x202BU));  // RLE
    AURORA_TEST_CHECK(is_bidi_format_control(0x202CU));  // PDF
    AURORA_TEST_CHECK(is_bidi_format_control(0x202DU));  // LRO
    AURORA_TEST_CHECK(is_bidi_format_control(0x202EU));  // RLO
    AURORA_TEST_CHECK(is_bidi_format_control(0x2066U));  // LRI
    AURORA_TEST_CHECK(is_bidi_format_control(0x2067U));  // RLI
    AURORA_TEST_CHECK(is_bidi_format_control(0x2068U));  // FSI
    AURORA_TEST_CHECK(is_bidi_format_control(0x2069U));  // PDI
    AURORA_TEST_CHECK(is_bidi_format_control(0x200EU));  // LRM
    AURORA_TEST_CHECK(is_bidi_format_control(0x200FU));  // RLM
    // 边界：相邻的 ZWSP / ZWNJ / BOM 不在其列（由 HarfBuzz 自行吞掉，无需特殊处理）。
    AURORA_TEST_CHECK(!is_bidi_format_control(0x200BU));  // ZWSP
    AURORA_TEST_CHECK(!is_bidi_format_control(0x200DU));  // ZWNJ
    AURORA_TEST_CHECK(!is_bidi_format_control(0xFEFFU));  // BOM
    // 普通可见字符不在其列。
    AURORA_TEST_CHECK(!is_bidi_format_control(U'A'));
    AURORA_TEST_CHECK(!is_bidi_format_control(U'ا'));  // 阿文 alef
    AURORA_TEST_CHECK(!is_bidi_format_control(U' '));
}

AURORA_TEST_CASE(bidi_format_control_zero_width_and_ink_font_independent) {
    // 用 TextInput 默认字体（捆绑 Noto，所有环境可用，不依赖外部 TTF，故不 SKIP），
    // 验证控制符在「度量」与「绘制」两侧都确定为零宽零墨迹：
    //  (a) 纯控制符串（不含任何可见字符）恒为零宽、零墨迹（即便字体无对应字形→.notdef 豆腐盒）；
    //  (b) 在「相同可见字母」中穿插控制符，可见文本的宽度/墨迹不变（控制符零宽不可见）。
    const aurora::Font font{.size_pt = 24.0F};
    const aurora::render::TextLayoutOpts ltr{.direction = aurora::TextDirection::LTR};
    const aurora::render::TextLayoutOpts rtl{.direction = aurora::TextDirection::RTL};
    ...
    for (const auto *opts : {&ltr, &rtl}) {
        const float w_vis = aurora::render::FontEngine::measure_width(visible, font, *opts);
        const float w_ctrl = aurora::render::FontEngine::measure_width(with_ctrl, font, *opts);
        AURORA_TEST_REQUIRE(w_vis > 0.0F);
        AURORA_TEST_CHECK_NEAR(w_vis, w_ctrl, 0.01F);  // 控制符不增加任何宽度
        const float w_pure = aurora::render::FontEngine::measure_width(pure_ctrl, font, *opts);
        AURORA_TEST_CHECK_NEAR(w_pure, 0.0F, 0.01F);  // 纯控制符串恒零宽
    }
    ...
    AURORA_TEST_CHECK_EQ(ink_vis, ink_ctrl);  // 控制符不引入/减少墨迹
