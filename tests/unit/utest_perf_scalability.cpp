...
AURORA_TEST() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // i∈[0,4)，无需边界检查
        const int n = AURORA_K_SIZES[i];  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
        snapshots[i] = run_benchmark(n, AURORA_K_FRAMES);  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
        auto const &s = snapshots[i];  // NOLINT
    ...
    for (int i = 1; i < 4; ++i) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
        const double ratio = snapshots[i].avg_frame_ms / (snapshots[i - 1].avg_frame_ms + 0.001);  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index) benchmark 固定大小数组，循环索引
        // i∈[0,4)，无需边界检查
