...
static void run() {
    ...
    // get_pixel 只读语义：界内返回写入值，越界返回 Color{}。
    {
        ...
        AURORA_TEST_CHECK(oob.r == 0 && oob.g == 0 && oob.b == 0 && oob.a == 0);
        const Color c = p3.get_pixel(3, 3);
        AURORA_TEST_CHECK(c.r == 10 && c.g == 20 && c.b == 30 && c.a == 255);
    }

    // 回归：圆角裁剪（慢路径，has_rounded_clip_=true 且无整窗边界保护）下，调用方传入
...
static auto blend_ref(Color dst, Color src) -> Color {
    ...
    return Color{blend(dst.r, src.r), blend(dst.g, src.g), blend(dst.b, src.b), 255};
}
...
static void run() {
    // 1) 不透明 + 半透明填充：与参考公式逐点一致（含非整数矩形边界）。
    {
        ...
        AURORA_TEST_CHECK(bg.r == 10 && bg.g == 200 && bg.b == 30);
        ...
        for (int y = 8; y <= 15; ++y) {
            for (int x = 10; x <= 27; ++x) {
                ...
                if (c.r != exp.r || c.g != exp.g || c.b != exp.b) {
                    inner_ok = false;
                }
            }
        }
        ...
        AURORA_TEST_CHECK(out.r == 10 && out.g == 200 && out.b == 30);
    ...
    //    clip = [10.0, 30.0]×[10.0, 20.0]：x=10..30、y=10..20（含端点）保留。
    {
        ...
        p.fill_rect(Rect{.origin = Point{.x = 0, .y = 0}, .size = Size{.width = 64, .height = 48}},
                    Color{0, 0, 0, 255});
        p.pop_clip();
        auto black = [&](int x, int y) -> bool { return p.get_pixel(x, y).r == 0; };
    ...
    // 3) 圆角裁剪走慢路径：角外像素被裁掉、中心保留（行为与历史一致）。
    {
        ...
        p.fill_rect(Rect{.origin = Point{.x = 0, .y = 0}, .size = Size{.width = 64, .height = 64}},
                    Color{0, 0, 0, 255});
        p.pop_clip();
        AURORA_TEST_CHECK(p.get_pixel(32, 32).r == 0);  // 中心被填充
        AURORA_TEST_CHECK(p.get_pixel(9, 9).r > 200);  // 圆角角外仍是白（被 SDF 裁掉）
    ...
    // 4) 全局透明度 <1 走慢路径：结果乘全局 alpha（与历史一致）。
    {
        ...
        AURORA_TEST_CHECK(c.r > 100 && c.r < 155);  // ≈127：半透明合成而非全黑覆写
...
static void run() {
    ...
    // ---- 1. draw_line：水平线中心实体、远处不受影响、端点外圆帽内 ----
    {
        ...
        AURORA_TEST_CHECK(edge.r > 200 && edge.g > 30 && edge.g < 240);
    }
...
auto row_mark(const Painter &p, int y) -> int { return p.get_pixel(0, y).r; }
...
static void run() {
    ...
    // 慢路径用「radius=0 的圆角裁剪」强制触发：has_rounded_clip_ 置位即回退逐像素分支，
