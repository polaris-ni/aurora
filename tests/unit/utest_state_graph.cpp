...
/// 测试说明: StateGraph 对活 State 的节点与 observes 边枚举、Effect dispose 与析构后的陈旧条目过滤，以及
/// to_json/to_text 与自由函数等价输出
...
AURORA_TEST_CASE(stale_entries_are_skipped_after_destruction) {
    ...
    {
        State<int> dead{0};
        Effect eff{[&dead]() -> void { (void)dead.get(); }};
        eff.run();
