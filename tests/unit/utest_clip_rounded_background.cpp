...
/// 测试说明: clip_rounded_background 单元测试
///
...
static auto region_has_bg(const Painter &p, int x0, int y0, int x1, int y1, const Color &bg) -> bool {
    ...
    for (int y = y0; y < y1; ++y) {
        for (int x = x0; x < x1; ++x) {
            ...
            const std::uint8_t *c = &p.data()[((static_cast<std::size_t>(y) * w) + x) * 4];  // NOLINT
...
static auto test_padded_rounded_background() -> void {
    ...
    auto sample = [&](int x, int y) -> Color {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        const uint8_t *px = p.data() + (((static_cast<std::size_t>(y) * iw) + x) * 4);  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        return Color{px[0], px[1], px[2], px[3]};  // NOLINT
    };
