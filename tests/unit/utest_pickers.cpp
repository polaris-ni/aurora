...
AURORA_TEST() {
    ...
    // ---- 9. 默认色板与选择 ----
    {
        ...
        AURORA_TEST_CHECK(cp->selected_color().r == 220);
        AURORA_TEST_CHECK(last.r == 220);

        // 相同颜色不重复回调
        last = Color{};
        cp->select(Color(220, 53, 69, 255));
        AURORA_TEST_CHECK(last.r == 0);
    }
    ...
    // ---- 10. 点击色板格选色（8 列 28dp 网格）----
    {
        ...
        AURORA_TEST_CHECK(cp->selected_color().g == 190);
    ...
    // ---- 11. 渲染与序列化 ----
    {
        ...
        AURORA_TEST_CHECK(cp2->selected_color().b == 255);
    }
}
