...
// ── API 覆盖映射 ─────────────────────────────
// WindowState(WindowState/WindowMode 正交枚举与映射，sec_window_state 段)。
...
struct SolidPage : au::Widget {
    au::Color bg;
    explicit SolidPage(au::Color c) : bg(c) {}
...
struct PhaseLeaf : au::LeafWidget {
    ...
    int m_layout_calls = 0;  ///< on_layout 累计运行次数
    int m_phase = 0;  ///< 外部状态：0=骨架, 1=真实内容（外部变更，不走 mark_needs_layout）
    bool m_saw_phase1 = false;  ///< on_layout 是否曾以 phase==1 运行（即拾取到了状态变更）
    ...
    [[nodiscard]] auto can_cache_layout() const -> bool override { return m_cache_layout; }

    auto on_layout(const au::Constraints &c, const au::BuildContext & /*ctx*/) -> Size override {
        ++m_layout_calls;
        if (m_phase == 1) {
            m_saw_phase1 = true;  // 仅在 on_layout 真正运行且已切到 phase=1 时置位
        }
        return c.constrain(c.max);
    }
...
static auto run_scenario(bool cache_layout) -> std::shared_ptr<PhaseLeaf> {
    ...
    leaf->m_cache_layout = cache_layout;
    auto page = Node{leaf};
...
void run() {
    ...
    AURORA_TEST_CHECK(old_leaf->m_layout_calls == 1);  // on_layout 被缓存冻结，不再运行
    AURORA_TEST_CHECK(!old_leaf->m_saw_phase1);  // 状态变更未被 on_layout 拾取（白屏根因）

    // ---- 修复（can_cache_layout==false）：缓存不在本控件命中
    //      → 强制重排每次重跑 on_layout，拾取 phase=1 → 不再冻结 ----
    const auto new_leaf = run_scenario(/*cache_layout=*/false);
    AURORA_TEST_CHECK(new_leaf->m_layout_calls > 1);  // 每次强制重排都重跑 on_layout
    AURORA_TEST_CHECK(new_leaf->m_saw_phase1);  // 状态变更被 on_layout 拾取（修复）
}
}  // namespace sec_window_layout_cache
...
/// 首帧 paint 时：先把 inner 标脏（登记到 dirty_boundaries_）再原地销毁 inner，
/// 使 dirty_boundaries_ 持有悬垂/失效指针进入下一帧。
struct BombHost : au::Widget {
    std::shared_ptr<Widget> inner;  ///< boundary 子节点（持有所有权）
    bool armed = true;  ///< 首帧 paint 触发「标脏 + 销毁」
    au::Color self_bg = au::Color::green();
    ...
    auto on_paint(au::Painter &p, const au::Rect &bounds, const au::BuildContext & /*ctx*/) -> void override {
        p.fill_rect(bounds, self_bg);
        if (armed && inner) {
            // 1) 标脏 → 触发 inner.on_dirty(true) → 登记 &inner 到 dirty_boundaries_（此时仍存活）
            inner->mark_needs_layout();
            // 2) 原地销毁 inner（模拟 lazy row 回收）→ dirty_boundaries_ 持有失效指针
            inner.reset();
            armed = false;
        }
    }
