...
AURORA_TEST() {
    ...
    const auto btn = std::make_shared<au::Button>("OK");
    const auto txt = std::make_shared<au::Text>("hello");
    const auto cb = std::make_shared<au::Checkbox>();
    const auto sw = std::make_shared<au::Switch>();
    const auto inner = std::make_shared<au::Column>(au::Column{au::Node{cb}, au::Node{sw}});
...
}  // namespace au::test_cases::utest_accessibility