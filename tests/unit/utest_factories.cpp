...
AURORA_TEST_CASE(button_adds_with_label_and_click) {
    aurora::Column root;
    int clicks = 0;
    aurora::Button *b = ui::button(root, "OK", aurora::ButtonProps{}, [&clicks] { ++clicks; });
...
AURORA_TEST_CASE(stack_grid_scroll_forward_props) {
    ...
    aurora::Scroll *sc =
        ui::scroll(root, aurora::ScrollProps{.child = aurora::Node(std::make_shared<aurora::Text>("page")), .step = 32.0F});
...
AURORA_TEST_CASE(lazy_row_and_list_forward_builder) {
    aurora::Column root;
    auto *lr = ui::lazy_row(root, 10, [](int i) { return aurora::Node(std::make_shared<aurora::Text>(std::to_string(i))); },
                            64.0F);
    AURORA_TEST_REQUIRE_NOT_NULL(lr);
    auto *ll = ui::lazy_list(root, 20, [](int i) { return aurora::Node(std::make_shared<aurora::Text>(std::to_string(i))); });
