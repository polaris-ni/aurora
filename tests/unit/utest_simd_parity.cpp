/// 测试类型: unit
/// 目标单元: include/aurora/render/detail/painter_simd.inl
/// 测试说明: simd_parity 单元测试
///
...
namespace aurora::test_cases::utest_simd_parity {

using au::detail::blend_linear_region;
using au::detail::blend_linear_region_scalar;
using au::detail::blend_srgb_over_region;
using au::detail::blend_srgb_over_region_scalar;
using au::detail::blur_region;
using au::detail::blur_region_scalar;
using au::detail::gradient_linear_fill;
using au::detail::gradient_linear_scanline_scalar;
using au::detail::gradient_radial_fill;
using au::detail::gradient_radial_scanline_scalar;
using au::detail::init_gamma_tables;
...
void fill_random(std::uint8_t *buf, const std::size_t n, std::uint32_t seed) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        buf[i] = static_cast<std::uint8_t>(dist(rng));  // NOLINT
    }
}
...
auto rgb_equal(const std::uint8_t *a, const std::uint8_t *b, int n) -> bool {
    for (int i = 0; i < n; ++i) {
        for (int c = 0; c < 3; ++c) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
            if (a[(static_cast<std::size_t>(i) * 4U) + c] != b[(static_cast<std::size_t>(i) * 4U) + c]) {  // NOLINT
                return false;
            }
        }
    }
