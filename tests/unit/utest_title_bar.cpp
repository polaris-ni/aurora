...
AURORA_TEST() {
    ...
    // ---- 11. TitleBar 控件：描述/序列化往返/Snap 计数/事件安全性 ----
    {
        ...
        // 序列化往返。
        bar.set_title("Document").set_subtitle("Modified").set_window_controls(false).set_height(44.0F);
        nlohmann::json j;
        bar.serialize_props(j);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
        // 容器类型无法本地确证为顺序容器，operator[] 与 .at() 语义不同（map/json 的 [] 会插入键）
        AURORA_TEST_CHECK(j["title"] == "Document");
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
        // 容器类型无法本地确证为顺序容器，operator[] 与 .at() 语义不同（map/json 的 [] 会插入键）
        ...
        AURORA_TEST_CHECK(bar2.title() == "Document");
        AURORA_TEST_CHECK(bar2.subtitle() == "Modified");
        ...
        int fired = 0;
        bar.add_snap_action({.label = "Fluent", .on_click = [&]() -> void { ++fired; }});
        ...
        AURORA_TEST_CHECK(press.is_handled);
    }
}
