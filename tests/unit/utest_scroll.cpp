...
static void run() {
    ...
    se.is_handled = false;
    multi.on_scroll(se);
    AURORA_TEST_CHECK(se.is_handled);
    ...
    ScrollEvent zero;
    zero.delta_y = 0.0F;
    zero.is_handled = false;
    tall.on_scroll(zero);
    AURORA_TEST_CHECK(zero.is_handled);

    // 14) 回归：on_scroll 偏移变化必须触发「仅绘制脏」重绘请求（request_frame(false)）。
    //     本框架脏区追踪下，present_root 实际绘制被裁剪到 dirty_；若滚动不标脏，则只有
...
struct ResizableStack : Widget {
  ...
  protected:
    ...
    auto on_paint(Painter &p, const Rect &bounds, const BuildContext & /*ctx*/) -> void override {
        ...
        const float oy = bounds.origin.y;  // Scroll 以 -buffer_origin_y_ 偏移把子控件绘进缓冲
...
auto paint_scroll(Node &root, BuildContext const &ctx, const int off = 0) -> Painter {
    ...
    root->layout(c, ctx);  // 重新布局以刷新 content_valid_ / 缓冲尺寸
    Painter p;
    p.begin(100, 100);
...
auto check_viewport_row(const Painter &p, int y, int offset, const char *msg) -> void {
    ...
    AURORA_TEST_CHECK_MSG(std::cmp_equal(p.get_pixel(5, y).r, expected), msg);
}
...
static void run() {
    ...
    // step 默认 16，scroll_by(-31.25) → offset_y_ += 31.25*16 = 500。
    ...
    // ---- (3) else 分支锚点重对齐（修复验证）：
    //     再滚动到 offset=300（设置 offset_y_，但不立即重绘），随后改内容高度触发缓冲失效，
    //     模拟「已滚动但缓冲未重锚点 + 内容尺寸变化」的到达态。buffer_origin_y_ 仍停留在
    //     上一帧的 400（offset=500 时），与当前 offset_y_=300 脱钩。
    //     若缺少 else 分支的对齐行：composite dy=400-300=100，视口映射到缓冲负行 → 整片空白；
    //     修复后：buffer_origin_y_ 重算为 200，视口显示 content[300,400)。
    scroll->scroll_by(12.5F);  // offset_y_: 500 → 500 - 12.5*16 = 300
    child->m_rows = 1200;  // 内容尺寸变化：下帧 content_valid_=false，但 buffer_origin_y_ 未更新
    {
        Painter p = paint_scroll(root, ctx);
...
struct SelfDrivingLeaf : LeafWidget {
    int paints = 0;
    Color color = AURORA_K_SKELETON_COLOR;
...
struct SwitchingBody : Container {
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        ...
        if (phase == 0) {
            leaf->color = AURORA_K_SKELETON_COLOR;
            skeleton = leaf;
        } else {
            leaf->color = AURORA_K_CONTENT_COLOR;
            content = leaf;
        }
        Node n{leaf};
        n.widget().mount(ctx);
...
auto count_color(const Surface &s, Color c, const int tol) -> long {
    ...
    for (int i = 0; i < w * h; ++i) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        const int r = buf[static_cast<std::size_t>(i) * 4];  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        const int g = buf[(i * 4) + 1];  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        const int b = buf[(i * 4) + 2];  // NOLINT
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // 测试助手：缓冲区长度已知且由断言约束，指针算术等价于 span 索引
        const int a = buf[(i * 4) + 3];  // NOLINT
        if (a > 0 && std::abs(r - c.r) <= tol && std::abs(g - c.g) <= tol && std::abs(b - c.b) <= tol) {
            ++n;
        }
    }
    return n;
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        ...
        if (!m_scroll) {
            ...
            children_.clear();
            children_.emplace_back(m_scroll);
            for (auto &n : children_) {
                n.widget().mount(ctx);
...
static void run() {
    ...
    AURORA_TEST_PRINTF("leaf paints=%d (total frames %d)\n", shell->m_leaf->paints, k_frames);
    AURORA_TEST_CHECK_MSG(shell->m_leaf->paints > k_frames / 2,
