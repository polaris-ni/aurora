...
/// 测试说明: reactive_lifecycle 单元测试
///

// 覆盖响应式内核（State ↔ Effect）观察者图的
// 生命周期安全。验证「任一侧先析构」都不会再解引用失效对象（此前为双向裸指针悬垂隐患）。
// ── API 覆盖映射 ─────────────────────────────
// Effect(Effect 析构安全/重跑)。
...
// T2：Effect 活得比 State 久（复现原 SEGFAULT 根因：旧 dispose 遍历 deps_ 触碰已死的 State）。
...
static void test_effect_outlives_state() {
    ...
    // s 已析构；e 仍存活，其 deps_ 持有对死 State 的（失效）引用
...
static void test_computed_lifecycle() {
    ...
    // s2 已析构；c2 仍存活，其内部 Effect 的 deps_ 持有失效引用
