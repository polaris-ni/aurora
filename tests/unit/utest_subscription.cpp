...
/// 测试说明: Subscription 句柄的活跃状态、幂等 reset、移动语义与 release 交接，以及 connect(State)/connect(Store) 的立即应用、增量回调与析构自动退订
...
AURORA_TEST_CASE(reset_invokes_cancel_once_and_is_idempotent) {
    int cancelled = 0;
    Subscription sub{[&cancelled] { ++cancelled; }};
...
AURORA_TEST_CASE(move_semantics_transfer_ownership_and_flush_pending_cancel) {
    ...
    int old_cancelled = 0;
    Subscription s3{[&old_cancelled] { ++old_cancelled; }};
    int new_cancelled = 0;
    Subscription s4{[&new_cancelled] { ++new_cancelled; }};
    s3 = std::move(s4);
...
AURORA_TEST_CASE(connect_state_applies_current_value_then_tracks_updates) {
    ...
    State<int> s{1};
    std::vector<int> seen;
    const auto sub = connect(s, [&seen](int v) { seen.push_back(v); });
...
AURORA_TEST_CASE(connect_state_subscription_scopes_to_destruction) {
    ...
    int calls = 0;
    {
        const auto sub = connect(s, [&calls](int) { ++calls; });
        s.set(1);
...
AURORA_TEST_CASE(connect_store_receives_new_state_on_dispatch) {
    ...
    std::vector<int> seen;
    {
        const auto sub = connect(store, [&seen](const int& v) { seen.push_back(v); });
