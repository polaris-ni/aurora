...
constexpr int CANVAS = 40;
constexpr int GOLDEN_SIZE = 64;
constexpr float HALF_PI = 1.57079632679489661923F;
constexpr float TWO_PI = 6.28318530717958647692F;
/// 图例联动把非高亮系列降到的透明度（0.35）对应的 alpha 通道值。
constexpr int DIM_ALPHA = 89;
...
AURORA_TEST_CASE(polyline_join_has_no_double_composite) {
    ...
    const Color dim{255, 0, 0, static_cast<std::uint8_t>(DIM_ALPHA)};
...
AURORA_TEST_CASE(polyline_records_and_replays_identically) {
    Painter direct;
    direct.begin(CANVAS, CANVAS);
    direct.stroke_polyline(elbow(), 4.0F, Color::red());
    direct.fill_sector(Point{.x = 20.0F, .y = 20.0F}, 12.0F, 5.0F, -HALF_PI, 0.0F, Color::blue());

    DisplayList dl;
    Painter recorded;
    recorded.begin(CANVAS, CANVAS);
    recorded.record(dl);
    recorded.stroke_polyline(elbow(), 4.0F, Color::red());
    recorded.fill_sector(Point{.x = 20.0F, .y = 20.0F}, 12.0F, 5.0F, -HALF_PI, 0.0F, Color::blue());
    recorded.stop();
    AURORA_TEST_CHECK_GT(static_cast<int>(dl.cmd_count()), 0);

    Painter replayed;
    replayed.begin(CANVAS, CANVAS);
    dl.replay(replayed);
...
AURORA_TEST_CASE(sector_fills_wedge_only) {
    ...
    p.fill_sector(Point{.x = 20.0F, .y = 20.0F}, 15.0F, 0.0F, -HALF_PI, 0.0F, Color::red());
...
AURORA_TEST_CASE(sector_inner_radius_forms_ring) {
    Painter p;
    p.begin(CANVAS, CANVAS);
    p.fill_sector(Point{.x = 20.0F, .y = 20.0F}, 15.0F, 8.0F, -HALF_PI, 0.0F, Color::red());
...
AURORA_TEST_CASE(sector_full_sweep_covers_all_quadrants) {
    Painter p;
    p.begin(CANVAS, CANVAS);
    p.fill_sector(Point{.x = 20.0F, .y = 20.0F}, 15.0F, 0.0F, -HALF_PI, -HALF_PI + TWO_PI, Color::red());
...
AURORA_TEST_CASE(stroke_arc_spans_thickness_band) {
    ...
    p.stroke_arc(Point{.x = 20.0F, .y = 20.0F}, 10.0F, 4.0F, -HALF_PI, 0.0F, Color::red());
...
AURORA_TEST_CASE(primitives_ignore_degenerate_inputs) {
    ...
    p.fill_sector(center, 0.0F, 0.0F, 0.0F, HALF_PI, Color::red());                          // 零外径
    p.fill_sector(center, 10.0F, 10.0F, 0.0F, HALF_PI, Color::red());                        // 内径 ≥ 外径
    p.fill_sector(center, 10.0F, 0.0F, HALF_PI, 0.0F, Color::red());                         // 角差 ≤ 0
    p.stroke_arc(center, 10.0F, 0.0F, 0.0F, HALF_PI, Color::red());                          // 零厚度
...
AURORA_TEST_CASE(golden_polyline_matches_baseline) {
    Painter p;
    p.begin(GOLDEN_SIZE, GOLDEN_SIZE);
    p.fill_rect(rect_at(0.0F, 0.0F, static_cast<float>(GOLDEN_SIZE), static_cast<float>(GOLDEN_SIZE)),
                Color::white());
    // 直角折线（join）+ 三点折线（细）+ 半透明折线（D13：顶点不得串珠）
    p.stroke_polyline(std::vector<Point>{Point{.x = 6.0F, .y = 8.0F},
                                         Point{.x = 30.0F, .y = 8.0F},
                                         Point{.x = 30.0F, .y = 28.0F}},
                      4.0F, Color::red());
    p.stroke_polyline(std::vector<Point>{Point{.x = 6.0F, .y = 40.0F},
                                         Point{.x = 18.0F, .y = 52.0F},
                                         Point{.x = 30.0F, .y = 40.0F},
                                         Point{.x = 42.0F, .y = 52.0F}},
                      1.5F, Color::blue());
    p.stroke_polyline(std::vector<Point>{Point{.x = 40.0F, .y = 8.0F},
...
AURORA_TEST_CASE(golden_sector_matches_baseline) {
    ...
    p.fill_sector(Point{.x = 16.0F, .y = 16.0F}, 14.0F, 0.0F, -HALF_PI, 0.0F, Color::red());
