...
/// 测试说明: UndoStack 的空栈不变量、push 执行 redo 与描述查询、undo/redo 光标往返、push 截断重做分支、深度上限丢弃最旧与 0 钳制、clear 清空，以及 macro 顺序重做/逆序撤销
...
AURORA_TEST_CASE(undo_redo_roundtrip_follows_cursor) {
    ...
    UndoStack stack;
    int value = 1;
    stack.push(UndoCommand{.redo = [&] { value = 2; }, .undo = [&] { value = 1; }, .description = "to 2"});
    stack.push(UndoCommand{.redo = [&] { value = 3; }, .undo = [&] { value = 2; }, .description = "to 3"});
...
AURORA_TEST_CASE(push_truncates_redo_branch) {
    ...
    int b_redo_runs = 0;
    int b_undo_runs = 0;
    stack.push(UndoCommand{.redo = [&] { value = 2; }, .undo = [&] { value = 1; }, .description = "A"});
    stack.push(UndoCommand{
        .redo = [&] {
            ++b_redo_runs;
            value = 3;
    ...
    stack.push(UndoCommand{.redo = [&] { value = 9; }, .undo = [&] { value = 2; }, .description = "C"});
...
AURORA_TEST_CASE(limit_drops_oldest_commands_on_overflow) {
    ...
    stack.push(UndoCommand{
        ...
        .description = "A"});
    stack.push(UndoCommand{.redo = [&] { value = 2; }, .undo = [&] { value = 1; }, .description = "B"});
    stack.push(UndoCommand{.redo = [&] { value = 3; }, .undo = [&] { value = 2; }, .description = "C"});
...
AURORA_TEST_CASE(set_limit_shrinks_history_and_clamps_zero) {
    ...
    UndoStack stack;
    int value = 0;
    stack.push(UndoCommand{.redo = [&] { value = 1; }, .undo = [&] { value = 0; }, .description = "A"});
    stack.push(UndoCommand{.redo = [&] { value = 2; }, .undo = [&] { value = 1; }, .description = "B"});
    stack.push(UndoCommand{.redo = [&] { value = 3; }, .undo = [&] { value = 2; }, .description = "C"});
    ...
    stack.push(UndoCommand{.redo = [&] { value = 9; }, .undo = [&] { value = 2; }, .description = "D"});
    stack.push(UndoCommand{.redo = [&] { value = 10; }, .undo = [&] { value = 9; }, .description = "E"});
...
AURORA_TEST_CASE(clear_resets_history_and_cursor) {
    ...
    stack.push(UndoCommand{.redo = [&] { value = 5; }, .undo = [&] { value = 0; }, .description = "after clear"});
    AURORA_TEST_CHECK_TRUE(stack.can_undo());
...
AURORA_TEST_CASE(macro_replays_redo_in_order_and_undo_in_reverse) {
    ...
    const UndoCommand first{.redo = [&] { trace.push_back("redo:a"); },
                            .undo = [&] { trace.push_back("undo:a"); },
                            .description = "a"};
    const UndoCommand second{.redo = [&] { trace.push_back("redo:b"); },
                             .undo = [&] { trace.push_back("undo:b"); },
                             .description = "b"};
