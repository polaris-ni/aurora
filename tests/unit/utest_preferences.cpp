...
static auto run_parent(const std::filesystem::path &exe, const std::filesystem::path &file) -> int {
    ...
    si.cb = sizeof(si);
    for (int i = 0; i < k; ++i) {
        const std::string cmd = "\"" + exe.string() + "\" --run=utest_preferences -- --writer " + std::to_string(i) +
                                " \"" + file.string() + "\"";
    ...
    for (int i = 0; i < K; ++i) {
        ...
        if (pid == 0) {
            ...
            ::execl(exe.c_str(), "test_preferences_multiproc", "--run=utest_preferences", "--", a1.c_str(), a2.c_str(),
...
static auto spawn(const std::filesystem::path &exe, const std::string &mode, const std::filesystem::path &file)
    ...
    const std::string cmd =
        "\"" + exe.string() + "\" --run=utest_preferences -- --" + mode + " \"" + file.string() + "\"";
    std::vector buf(cmd.begin(), cmd.end());
    buf.push_back('\0');
    STARTUPINFOA si{};
    si.cb = sizeof(si);
...
static auto spawn(const std::filesystem::path &exe, const std::string &mode, const std::filesystem::path &file)
    ...
    if (pid == 0) {
        ...
        ::execl(exe.c_str(), "test_preferences_multiproc_delete", "--run=utest_preferences", "--", a1.c_str(),
...
AURORA_TEST() {
    // 子进程模式派发：runner 父进程会以「runner --run=utest_preferences -- 模式参数」重启子进程；
