...
struct FixedBox : Widget {
    Size sz;
    Color bg;
    FixedBox(float w, float h, Color c) : sz{.width = w, .height = h}, bg(c) {}
...
struct Placed : SingleChild {
    Point at;
    Size sz;
    Placed(Node child, Point a, Size s) : SingleChild(std::move(child)), at(a), sz(s) {}
  ...
  protected:
    auto on_layout(const Constraints &c, const BuildContext &ctx) -> Size override {
        child_.widget().layout(c, ctx);
        return sz;
    }
    auto on_paint(Painter &p, const Rect &bounds, const BuildContext &ctx) -> void override {
        child_.widget().paint(p, Rect{.origin = bounds.origin + at, .size = sz}, ctx);
    }
};
