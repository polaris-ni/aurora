...
constexpr int AURORA_CAP = 1024;
constexpr int AURORA_CH = 2;

/// 生成交错帧序列：第 i 帧两声道为 (i, i+1000)，便于验证回绕后的段拼接顺序。
auto make_block(int frames) -> std::vector<float> {
    std::vector<float> out(static_cast<std::size_t>(frames) * AURORA_CH);
    for (int i = 0; i < frames; ++i) {
        out[static_cast<std::size_t>(i) * AURORA_CH] = static_cast<float>(i);
        out[(static_cast<std::size_t>(i) * AURORA_CH) + 1] = static_cast<float>(i + 1000);
    }
    return out;
}
...
auto read_out(aurora::detail::WebAudioRing &ring, int frames, std::vector<float> &dst) -> int {
    ...
    for (int i = 0; i < n; ++i) {
        for (int c = 0; c < stride; ++c) {
            // 环形交错 PCM 裸指针按下标写入是 JS/wasm 内存交接边界，指针算术不可避免。
            // NOLINTBEGIN(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            dst[(static_cast<std::size_t>(i) * stride) + c] =
                data[(static_cast<std::size_t>((t + i) % cap) * stride) + c];
...
auto zero_render(float *out, int frames) -> void {
    std::fill_n(out, static_cast<std::size_t>(frames) * AURORA_CH, 0.0F);
}
...
AURORA_TEST_CASE(ring_normalizes_degenerate_geometry) {
    ...
    const aurora::detail::WebAudioRing a(0, AURORA_CH);
    AURORA_TEST_CHECK_EQ(a.capacity_frames(), 2);
    const aurora::detail::WebAudioRing b(AURORA_CAP, 0);
    AURORA_TEST_CHECK_EQ(b.channels(), 1);
    AURORA_TEST_CHECK_EQ(b.capacity_frames(), AURORA_CAP);
}

AURORA_TEST_CASE(ring_keeps_one_slot_free_to_tell_full_from_empty) {
    aurora::detail::WebAudioRing ring(AURORA_CAP, AURORA_CH);
    AURORA_TEST_CHECK_EQ(ring.avail_frames(), 0);
    AURORA_TEST_CHECK_EQ(ring.free_frames(), AURORA_CAP - 1);  // 恒留一格 ⇒ 满/空可辨
    const auto block = make_block(AURORA_CAP);  // 故意多写一格
    AURORA_TEST_CHECK_EQ(ring.write(block.data(), AURORA_CAP), AURORA_CAP - 1);  // 裁剪到可用容量
    AURORA_TEST_CHECK_EQ(ring.free_frames(), 0);
    AURORA_TEST_CHECK_EQ(ring.write(block.data(), 1), 0);  // 满则拒写，绝不覆盖未读数据
    AURORA_TEST_CHECK_EQ(ring.avail_frames(), AURORA_CAP - 1);
}
...
AURORA_TEST_CASE(ring_wraps_without_scrambling_interleaved_frames) {
    ...
    // ——任何错位都是耳朵听不出的爆音源。
    aurora::detail::WebAudioRing ring(AURORA_CAP, AURORA_CH);
    const auto first = make_block(AURORA_CAP - 4);
    AURORA_TEST_CHECK_EQ(ring.write(first.data(), AURORA_CAP - 4), AURORA_CAP - 4);
    std::vector<float> drained;
    AURORA_TEST_CHECK_EQ(read_out(ring, AURORA_CAP - 4 - 1, drained), AURORA_CAP - 5);  // 留 1 帧在环里
    ring.sync_reader();
    ...
    AURORA_TEST_CHECK_EQ(got[0], static_cast<float>(AURORA_CAP - 5));  // 残留帧仍是上一块的末帧
    AURORA_TEST_CHECK_EQ(got[1], static_cast<float>(AURORA_CAP - 5 + 1000));
    for (int i = 1; i <= 8; ++i) {
        AURORA_TEST_CHECK_EQ(got[static_cast<std::size_t>(i) * AURORA_CH], static_cast<float>(i - 1));
        AURORA_TEST_CHECK_EQ(got[(static_cast<std::size_t>(i) * AURORA_CH) + 1], static_cast<float>(i - 1 + 1000));
    }
...
AURORA_TEST_CASE(ring_accounts_reader_advance_across_wrap) {
    aurora::detail::WebAudioRing ring(AURORA_CAP, AURORA_CH);
    const auto block = make_block(AURORA_CAP - 1);
    AURORA_TEST_CHECK_EQ(ring.write(block.data(), AURORA_CAP - 1), AURORA_CAP - 1);
    std::vector<float> got;
    read_out(ring, AURORA_CAP - 1, got);  // 读指针回绕到 0
    ring.sync_reader();
    AURORA_TEST_CHECK_EQ(ring.consumed_frames(), AURORA_CAP - 1);
    AURORA_TEST_CHECK_EQ(ring.avail_frames(), 0);
...
AURORA_TEST_CASE(device_format_and_gauge_stay_in_contract) {
    ...
    if (aurora::WebAudioDeviceBackend::context_state() == -1) {
