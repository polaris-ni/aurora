...
/// 测试说明: 覆盖 SpringDescription 派生量、SpringSimulation 三种阻尼 regimes 的端点/收敛、临界阻尼无过冲、欠阻尼过冲回落、初速度塑形、数值微分速度与 is_settled 的位置+速度双条件
...
AURORA_TEST_CASE(underdamped_overshoots_then_converges) {
    const aurora::SpringDescription under{100.0, 2.0, 1.0};  // ζ=0.1, ω₀=10
    const aurora::SpringSimulation sim{under, 0.0, 1.0};

    const double peak_t = 3.14159265358979323846 / (10.0 * 0.99498743710662);  // π/ωd ≈ 0.3157
