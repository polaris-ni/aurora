...
/// 测试说明: custom_surface 单元测试
///

// 覆盖自定义 Surface 注入路径与后端代码剪裁（feature 宏）契约。
// 用例经 AURORA_TEST() 注册，main 与汇总由 runner（aurora_test_main.cpp）统一提供。
// ── API 覆盖映射 ─────────────────────────────
// Surface(Surface 抽象契约：自定义 Surface 注入/feature 宏剪裁)。
...
class CustomSurface : public au::Surface {
  public:
    auto begin_frame(int w, int h) -> au::Result<bool> override {
        ...
        m_size_ = au::Size{.width = static_cast<float>(w), .height = static_cast<float>(h)};
        return au::Result{true};
    }
    auto painter() -> au::Painter & override { return m_painter_; }
    auto present() -> au::Result<bool> override {
        ++m_frames_;
        return au::Result{true};
    }
    [[nodiscard]] auto size() const -> au::Size override { return m_size_; }
    [[nodiscard]] auto frame_count() const -> int override { return m_frames_; }

  private:
    au::Painter m_painter_;
    au::Size m_size_{.width = 0.0F, .height = 0.0F};
    int m_frames_ = 0;
};
}  // namespace
