...
static void run() {
    ...
    // 1) 命中落点 (5,5) 处控件应收到事件，路径与坐标正确。
    {
        ...
        AURORA_TEST_CHECK(catcher->paths.at(0) == "/tmp/a.png");
        AURORA_TEST_CHECK(catcher->paths.at(1) == "/tmp/b.txt");
    ...
    // 3) Application 级便捷派发：经 dispatch 路径触发 on_file_drop。
    {
        ...
        AURORA_TEST_CHECK(catcher2->paths.at(0) == "/home/file.dat");
    }
}
}  // namespace sec_file_drop
