...
/// 测试说明: Computed<T> 构造求值与缓存、依赖变化重算、Effect 通知、链式派生、条件依赖分支、异常传播与 computed
/// 工厂类型推导
...
AURORA_TEST_CASE(computed_recomputes_on_dependency_change) {
    ...
    State<int> n{1};
    Computed<int> c{[&]() -> int { return n.get() + 1; }};
...
AURORA_TEST_CASE(computed_notifies_dependent_effect) {
    ...
    State<int> a{1};
    Computed<int> c{[&]() -> int { return a.get() * 2; }};
    int runs = 0;
    int last = 0;
    Effect e{[&]() -> void {
        ++runs;
...
AURORA_TEST_CASE(computed_chained_derivation_propagates) {
    ...
    State<int> base{1};
    auto up = computed([&]() -> int { return base.get() + 1; });
    Computed<int> down{[&]() -> int { return up.get() * 10; }};
...
AURORA_TEST_CASE(computed_conditional_reads_follow_current_branch) {
    ...
    State<int> x{1};
    State<int> y{2};
    Computed<int> c{[&]() -> int { return mode.get() != 0 ? x.get() : y.get(); }};
...
AURORA_TEST_CASE(computed_throwing_fn_propagates_from_construction) {
    ...
    State<int> hot{1};
    AURORA_TEST_CHECK_THROW(Computed<int>{[&]() -> int {
                                if (hot.get() > 0) {
    ...
    // 同一 State 仍可正常派生。
    Computed<int> c{[&]() -> int { return hot.get() * 2; }};
    AURORA_TEST_CHECK_EQ(c.get(), 2);
    hot.set(3);
    AURORA_TEST_CHECK_EQ(c.get(), 6);
}
...
AURORA_TEST_CASE(computed_factory_deduces_value_type) {
    ...
    State<int> n{3};
    auto doubled = computed([&]() -> int { return n.get() * 2; });
    ...
    auto shout = computed([&]() -> std::string { return word.get() + "!"; });
