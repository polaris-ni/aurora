...
static void test_localized_string() {
    ...
    AURORA_TEST_CHECK_MSG(plain.resolve(nullptr, Locale{"en"}) == "Hi", "LocalizedString: non-localized returns text");
...
static void test_i18n_runtime() {
    ...
    const std::string r2 = t.resolve(LocalizedString::tr("items", {LocalizedString{"1"}}), Locale{.language = "en"});
    AURORA_TEST_CHECK_MSG(r2 == "1 item", "i18n plural one");

    const std::string r3 = t.resolve(LocalizedString::tr("items", {LocalizedString{"3"}}), Locale{.language = "en"});
