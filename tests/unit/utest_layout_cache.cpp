...
AURORA_TEST() {
    ...
    AURORA_TEST_CHECK_EQ(a->layout_calls, 1);
    AURORA_TEST_CHECK_EQ(b->layout_calls, 1);
    ...
    AURORA_TEST_CHECK_EQ(a->layout_calls, 2);
    AURORA_TEST_CHECK_EQ(b->layout_calls, 1);
    ...
    // RelayoutBoundary 控件标脏后仍能正确失效并重排（缓存不污染固定尺寸控件）。
    {
        ...
        AURORA_TEST_CHECK_EQ(boundary->layout_calls, 1);
