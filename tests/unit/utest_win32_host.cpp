/// 测试类型: unit
/// 目标单元: include/aurora/window/win32_host.h
...
namespace aurora::test_cases::utest_win32_host {

AURORA_TEST_CASE(win32_host_type_contract) {
#ifdef AURORA_BACKEND_WIN32
    // 契约：Win32Host 是独立窗口宿主（不含像素上屏），并非 Surface 派生类；
    // 与 Win32Surface(GDI)/D3D11Surface(GPU) 组合而非继承。
    static_assert(!std::is_base_of_v<aurora::Surface, aurora::Win32Host>);
    static_assert(std::is_class_v<aurora::Win32Host>);
    static_assert(!std::is_copy_constructible_v<aurora::Win32Host>);
    static_assert(!std::is_move_constructible_v<aurora::Win32Host>);
...
AURORA_TEST_CASE(win32_host_creation_skipped) {
#ifdef AURORA_BACKEND_WIN32
