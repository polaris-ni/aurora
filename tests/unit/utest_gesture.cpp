...
static void test_touch_event_basic() {
    ...
    e.points.push_back(TouchPoint{
        .id = 0, .position = Point{.x = 100, .y = 100}, .prev_position = Point{.x = 100, .y = 100}, .is_active = true});
    AURORA_TEST_CHECK(e.active_count() == 1);

    e.points.push_back(TouchPoint{
        .id = 1, .position = Point{.x = 200, .y = 100}, .prev_position = Point{.x = 200, .y = 100}, .is_active = true});
