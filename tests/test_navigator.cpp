...
static void run() {
    ...
    // ---- 7. open_uri（RouteRegistry 版） ----
    {
        ...
        registry["home"] = [](const std::string &) -> Route {
            return Route{ Node{ SolidPage{ Color::green() } }, "home" };
        };
...
static void run() {
    ...
    AURORA_TEST_CHECK_MSG(f_n > 1000, "under real AppShell, banner after entrance animation should show color gradient "
