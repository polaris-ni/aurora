...
struct PurityProbe : au::Widget {
    int depth_in_paint = -1;
    int depth_in_layout = -1;
  ...
  protected:
    auto on_layout(const au::Constraints &c, const au::BuildContext &ctx) -> au::Size override {
        (void)ctx;
        depth_in_layout = au::debug::g_paint_depth; // layout 不在绘制上下文中，应为 0
        return c.max;
    }
    auto on_paint(au::Painter & /*p*/, const au::Rect & /*bounds*/, const au::BuildContext & /*ctx*/) -> void override {
        depth_in_paint = au::debug::g_paint_depth; // paint 中应为 >0
    }
};
} // namespace
...
static void test_paint_purity_flag() {
    ...
    w.layout(cc, ctx);
    AURORA_TEST_CHECK_MSG(w.depth_in_layout == 0, "g_paint_depth == 0 during layout (not in paint)");
    au::Painter p;
    p.begin(100, 100);
    w.paint(
        p,
        au::Rect{ .origin = au::Point{ .x = 0.0f, .y = 0.0f }, .size = au::Size{ .width = 100.0f, .height = 100.0f } },
        ctx);
    AURORA_TEST_CHECK_MSG(w.depth_in_paint > 0, "g_paint_depth > 0 during Widget::paint (purity guard active)");
