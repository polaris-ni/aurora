...
constexpr int AURORA_LEAF_COUNT = 10;
constexpr float AURORA_CANVAS_W = 200.0F;
constexpr float AURORA_CANVAS_H = 200.0F;
...
AURORA_TEST_CASE(recording_captures_draw_commands) {
    // 1) 录制确实产出命令（不为空）。
    const std::size_t count = record_cmd_count(build_tree(AURORA_LEAF_COUNT), AURORA_CANVAS_W, AURORA_CANVAS_H);
    AURORA_TEST_CHECK_MSG(count > 0, "DL cmd_count > 0 (recording captured draw commands)");
    AURORA_TEST_PRINTF("  cmd_count: N=%d -> %zu\n", AURORA_LEAF_COUNT, count);
}

AURORA_TEST_CASE(identical_tree_records_deterministic_count) {
    // 2) 相同树两次独立录制 → 命令数可复现（计数类指标必须确定，不能逐次漂移）。
    const std::size_t first = record_cmd_count(build_tree(AURORA_LEAF_COUNT), AURORA_CANVAS_W, AURORA_CANVAS_H);
    const std::size_t second = record_cmd_count(build_tree(AURORA_LEAF_COUNT), AURORA_CANVAS_W, AURORA_CANVAS_H);
...
AURORA_TEST_CASE(command_count_grows_with_content) {
    ...
    const std::size_t base = record_cmd_count(build_tree(AURORA_LEAF_COUNT), AURORA_CANVAS_W, AURORA_CANVAS_H);
    const std::size_t doubled = record_cmd_count(build_tree(AURORA_LEAF_COUNT * 2), AURORA_CANVAS_W, AURORA_CANVAS_H);
...
AURORA_TEST_CASE(command_count_scales_sublinearly) {
    ...
    AURORA_TEST_PRINTF("  cmd_count: N=%d -> %zu, N=%d -> %zu\n", AURORA_LEAF_COUNT, base, AURORA_LEAF_COUNT * 2,
                       doubled);
}
