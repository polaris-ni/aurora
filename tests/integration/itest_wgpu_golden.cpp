...
///
///           不经 E2E 内核（`e2e::Session`）的理由：本 TU 验的是 **RHI 离屏重放**
///           （`native_window = nullptr` + 离屏尺寸），不建窗口、不推进 `present_root`，读回走
///           `RhiFrameSink::read_pixels`（离屏 FBO），而内核的窗口面读回基底是 `Surface::data()`
///           ——两者是不同层的读回通道；本 TU 与 GL 侧共用 `support/gpu_golden_scenes.h` 的场景
///           与帧装配、共用 `golden::compare_gpu_tolerance` 判据，已是单源。
...
AURORA_TEST_CASE(gpu_polyline_within_tolerance_of_software_golden) {
    AURORA_ITEST_WGPU_GPU_OR_SKIP(gpu);
