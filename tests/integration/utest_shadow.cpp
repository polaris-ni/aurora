...
/// 测试说明: utest_shadow 单元测试

// 阴影绘制与 Modifier.shadow() 测试。
#include <vector>
...
static void test_shadow_hard() {
    ...
    AURORA_TEST_CHECK(shadow_area.r < 200);  // 被阴影染暗

    // 远离阴影的区域应保持白色
    const Color far = p.get_pixel(90, 10);
    AURORA_TEST_CHECK(far.r > 250);
}
...
static void test_shadow_blur() {
    ...
    AURORA_TEST_CHECK(edge.r < 255);  // 有些暗
    AURORA_TEST_CHECK(edge.r > 150);  // 但不是很暗（衰减了）
}
...
static void test_modifier_shadow() {
    ...
    for (const auto &n : mod.nodes()) {
        if (const auto *s = dynamic_cast<const ShadowNode *>(n.get())) {
            ...
            AURORA_TEST_CHECK(s->color().a == 80);
        }
    }
...
static void test_modifier_shadow_defaults() {
    ...
    for (const auto &n : mod.nodes()) {
        if (const auto *s = dynamic_cast<const ShadowNode *>(n.get())) {
            ...
            AURORA_TEST_CHECK(s->color().a == 64);
        }
    }
    AURORA_TEST_CHECK(found);
}
