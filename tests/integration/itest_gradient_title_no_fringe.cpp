...
/// 测试说明: 多色渐变背景（蓝→紫 24-band，仿 GradientTitle）上白色文字的 AA 模式对比留证：
///           ClearType 用单点 bg 决定子像素着色 → 字形像素 R/G/B 通道差异大（红/蓝羽化），
///           Supersample 灰度 AA → 字形像素近似灰度。逐像素统计两种 AA 下同一坐标的通道差与
///           通道不平衡指标（演示型，指标仅输出不断言，与旧 utest 一致）
/// 覆盖说明: 另断言文字区外的渐变带底色像素不被文字污染（确定性回归锚点）
...
namespace aurora::test_cases::itest_gradient_title_no_fringe {
...
AURORA_TEST_CASE(clear_type_vs_supersample_metrics_on_gradient) {
    constexpr int w = 600;
    constexpr int h = 80;
    const auto ct = render_one(ar::TextAAMode::ClearType, w, h);
    const auto ss = render_one(ar::TextAAMode::Supersample, w, h);

    // 确定性锚点：文字矩形（x∈[20,580], y∈[20,60]）之外，渐变带底色不被文字污染。
    // (5,5) 落在第 0 带（蓝 37,99,235）且在文字矩形外；两模式结果一致。
    const auto px = static_cast<std::size_t>((5 * w) + 5) * 4U;
    AURORA_TEST_CHECK_EQ(ct.at(px + 0), 37);
    AURORA_TEST_CHECK_EQ(ct.at(px + 1), 99);
    AURORA_TEST_CHECK_EQ(ct.at(px + 2), 235);
    AURORA_TEST_CHECK_EQ(ct.at(px), ss.at(px));
    ...
    for (std::size_t i = 0; i < n_px; ++i) {
        ...
        // 更直接的检测：在 ClearType 下 R-B/R-G/G-B 通道差与在 Supersample 下差异巨大。
    ...
    // 演示型测试：仅输出指标，不断言羽化阈值。这是 ClearType 红/蓝羽化在多变背景上的"留证"——
