...
AURORA_TEST_CASE(codegen_scalar_props_string_bool_float_int) {
    ...
    AURORA_TEST_CHECK_MSG(float_code.find(".font_size = ") != std::string::npos, "emit_props: float prop present");
...
AURORA_TEST_CASE(codegen_length_forms_px_percent_legacy_auto_fill) {
    ...
    AURORA_TEST_CHECK_MSG(legacy_pct.find("au::percent(") != std::string::npos, "emit_props: Length legacy object pct");

    const std::string auto_fill = to_code(make_node("Text", au::Json{{"width", "auto"}, {"height", "fill"}}));
...
AURORA_TEST_CASE(codegen_color_and_edge_insets_values) {
    ...
    const std::string insets = to_code(make_node(
        "Column", au::Json{{"padding", au::Json{{"top", 8.0F}, {"right", 12.0F}, {"bottom", 8.0F}, {"left", 12.0F}}}}));
...
AURORA_TEST_CASE(codegen_enum_keys_emit_enum_expressions) {
    ...
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Text", au::Json{{"text_align", "Center"}})).find("TextAlign::Center") != std::string::npos,
        "emit_props: enum TextAlign");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Text", au::Json{{"text_overflow", "Ellipsis"}})).find("TextOverflow::Ellipsis") !=
            std::string::npos,
    ...
    AURORA_TEST_CHECK_MSG(to_code(make_node("Column", au::Json{{"main_axis_size", "Max"}})).find("MainAxisSize::Max") !=
                              std::string::npos,
                          "emit_props: enum MainAxisSize");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Stack", au::Json{{"stack_fit", "Expand"}})).find("StackFit::Expand") != std::string::npos,
        "emit_props: enum StackFit");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Image", au::Json{{"box_fit", "Cover"}})).find("BoxFit::Cover") != std::string::npos,
        "emit_props: enum BoxFit");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Column", au::Json{{"overflow_strategy", "Scroll"}})).find("OverflowStrategy::Scroll") !=
            std::string::npos,
        "emit_props: enum OverflowStrategy");

    // 真实序列化属性名（to_json 的产出键名）。
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Stack", au::Json{{"alignment", "TopLeft"}})).find("Alignment::TopLeft") != std::string::npos,
        "emit_props: real key alignment → Alignment");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Text", au::Json{{"overflow", "Ellipsis"}})).find("TextOverflow::Ellipsis") !=
            std::string::npos,
        "emit_props: real key overflow → TextOverflow");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("Drawer", au::Json{{"side", "Right"}})).find("DrawerSide::Right") != std::string::npos,
        "emit_props: real key side → DrawerSide");
    AURORA_TEST_CHECK_MSG(
        to_code(make_node("ToastHost", au::Json{{"position", "Top"}})).find("ToastPosition::Top") != std::string::npos,
...
AURORA_TEST_CASE(codegen_font_weight_style_and_text_decoration) {
    ...
    AURORA_TEST_CHECK_MSG(bold.find("FontWeight::Bold") != std::string::npos, "emit_props: FontWeight 700 → Bold");
    ...
    AURORA_TEST_CHECK_MSG(italic.find("FontStyle::Italic") != std::string::npos, "emit_props: enum FontStyle");

    const std::string single =
        to_code(make_node("Text", au::Json{{"text_decoration", au::Json::array({"Underline"})}}));
    AURORA_TEST_CHECK_MSG(single.find("TextDecoration::Underline") != std::string::npos,
                          "emit_props: TextDecoration array Underline");
    const std::string combined =
        to_code(make_node("Text", au::Json{{"text_decoration", au::Json::array({"Underline", "LineThrough"})}}));
    ...
    AURORA_TEST_CHECK_MSG(none.find("TextDecoration::None") != std::string::npos, "emit_props: TextDecoration None");
}

AURORA_TEST_CASE(codegen_all_styles_share_value_emission) {
    // Color / 枚举 / EdgeInsets 在三种风格下的表达式发射一致。
    const au::Json color_node = make_node("Text", au::Json{{"color", au::Json::array({0, 128, 255, 255})}});
    ...
    AURORA_TEST_CHECK_MSG(to_code(enum_node, CodeStyle::Fluent).find("TextAlign::Right") != std::string::npos,
                          "styles: Fluent enum");
    AURORA_TEST_CHECK_MSG(to_code(enum_node, CodeStyle::DesignatedInit).find("TextAlign::Right") != std::string::npos,
                          "styles: DesignatedInit enum");
    AURORA_TEST_CHECK_MSG(to_code(enum_node, CodeStyle::StepByStep).find("TextAlign::Right") != std::string::npos,
                          "styles: StepByStep enum");

    const au::Json insets_node = make_node(
        "Column", au::Json{{"padding", au::Json{{"top", 4.0F}, {"right", 8.0F}, {"bottom", 4.0F}, {"left", 8.0F}}}});
    AURORA_TEST_CHECK_MSG(to_code(insets_node, CodeStyle::Fluent).find("EdgeInsets{") != std::string::npos,
                          "styles: Fluent EdgeInsets");
    AURORA_TEST_CHECK_MSG(to_code(insets_node, CodeStyle::StepByStep).find("EdgeInsets{") != std::string::npos,
