...
/// 测试说明: utest_prop_validation 单元测试

// 属性约束验证测试：validate_prop<T> 各特化 + Diagnostics 报告 + Widget::validate_props。
...
namespace aurora::test_cases::utest_prop_validation {
...
static void test_validate_color() {
    ...
    // 合法：4 元素数组，值在 0-255
    {
        ...
        AURORA_TEST_CHECK_MSG(r.ok() && r.value().r == 255 && r.value().g == 128,
