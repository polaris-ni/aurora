/// 测试类型: unit
/// 目标单元: (待补)
/// 测试说明: aurora_lsp 单元测试
///

// 覆盖 #1 aurora-lsp 的轻量分析器逻辑（completion / hover /
// diagnostics / codeAction）。直接驱动 tools/include/lsp_features.h（汇聚 schema/document/features 三层），不依赖真实
// LSP socket。 用例经 AURORA_TEST() 注册，main 与汇总由 runner（aurora_test_main.cpp）统一提供。
#include <string>

#include "../tools/include/lsp_features.h"
#include "aurora_test_harness.h"

namespace aurora::test_cases::utest_aurora_lsp {
