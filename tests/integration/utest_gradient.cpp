...
/// 测试说明: utest_gradient 单元测试

// 渐变绘制与 Modifier.gradient() 测试。
#include <cmath>
#include <vector>
...
static void test_linear_gradient_basic() {
    ...
    AURORA_TEST_CHECK(left.r > 200);
    AURORA_TEST_CHECK(left.b < 50);

    // 右侧像素应偏蓝
    const Color right = p.get_pixel(95, 50);
    AURORA_TEST_CHECK(right.r < 50);
    AURORA_TEST_CHECK(right.b > 200);

    // 中间像素应为紫色混合
    const Color mid = p.get_pixel(50, 50);
    AURORA_TEST_CHECK(mid.r > 80 && mid.r < 180);
    AURORA_TEST_CHECK(mid.b > 80 && mid.b < 180);
}
...
static void test_linear_gradient_vertical() {
    ...
    AURORA_TEST_CHECK(top.r > 230);

    // 底部应暗
    const Color bottom = p.get_pixel(25, 95);
    AURORA_TEST_CHECK(bottom.r < 25);
}
...
static void test_linear_gradient_multi_stop() {
    ...
    AURORA_TEST_CHECK(mid.g > 200);
    AURORA_TEST_CHECK(mid.r < 50);
    AURORA_TEST_CHECK(mid.b < 50);
}
...
static void test_radial_gradient_basic() {
    ...
    AURORA_TEST_CHECK(center.r > 230);

    // 边缘应暗
    const Color edge = p.get_pixel(5, 50);
    AURORA_TEST_CHECK(edge.r < 50);
}
